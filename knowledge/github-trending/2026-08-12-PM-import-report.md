# 源码导入报告：2026-08-12 PM

此报告由 GitHub Actions 生成。导入的是受限源码快照，不执行第三方代码。

| 项目 | 状态 | 目录或原因 |
| --- | --- | --- |
| `github/github-mcp-server` | updated | `mcp/servers/github-mcp-server @ 316b8efcd05b849e3f97ab37129083240f8e4c21 (485 files, 4564051 bytes); excluded 62 files` |
| `ark-forge/mcp-eu-ai-act` | up-to-date | `mcp/security/mcp-eu-ai-act @ 980e749d188d7e9b6d31c2d6c7f29d713cedd806` |
| `UKGovernmentBEIS/inspect_ai` | up-to-date | `agent/evaluations/inspect-ai @ 1e7082c897f0324c7269f0703a85d4be881a34f7` |
| `sattyamjjain/agent-audit-kit` | up-to-date | `knowledge/security/agent-audit-kit @ 56cecf4f4ff4b70d5a70e30dde6b1443b1487107` |
| `OWASP/www-project-top-10-for-large-language-model-applications` | up-to-date | `knowledge/standards/owasp-genai-llm-top10 @ 99f4395589bdbd120ae961f9cd179e79d7f9b27f` |
