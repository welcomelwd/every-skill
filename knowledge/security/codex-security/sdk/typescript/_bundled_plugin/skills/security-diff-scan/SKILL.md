...
5. Record findings, coverage, and the retained canonical model with `record_codex_security_scan_draft({ scanId, handoffClaimToken?, scope?, threatModel, findings, coverage })`. Mark unresolved work as deferred. Request detailed write-ups or hardening plans only when the user asks.
