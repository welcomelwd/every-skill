...
Make one call and wait for it. Discovery stops dispatching new workers after the configured `[deep_scan].max_time_hours` duration, cancels unfinished discovery, merges every completed discovery, and returns the existing `capped` terminal manifest. The default and maximum configured duration are 96 hours, reserving approximately one hour for finalization under the 97-hour tool-call timeout. Then continue immediately with centralized validation, attack-path analysis, and reporting. The call otherwise returns only after discovery completes, fails, or is canceled. The tool owns the transition into the discovery phase, so leave the public scan phase at preflight before calling it. Do not publish discovery progress yourself while the call is pending.
...
Do not read live worker state, repair worker artifacts, or redo discovery. If a required manifest field or shared discovery artifact is missing or malformed, report the tool failure and stop before validation. An empty compact candidate ledger with completed discovery workers is the ordinary no-findings discovery result; it still requires a valid terminal manifest. If the manifest is `capped` with `discoveryCount: 0`, no source review completed: treat its empty ledger as incomplete discovery, never as evidence that the repository has no vulnerabilities.
...
If the manifest is `capped` with `discoveryCount: 0`, do not invent or synthesize a threat model. Publish the ordinary forward phase transitions and close the empty phases directly with `record_codex_security_candidate_validations({ scanId, validations: [] })` and `record_candidate_attack_paths({ scanId, attackPaths: [] })`. Record `record_codex_security_scan_draft({ scanId, handoffClaimToken?, findings: [], coverage: { completeness: "partial", surfaces: [], explicitExclusions: [], deferred: [{ reason: "The configured discovery time limit elapsed before any source review completed." }] } })` without `threatModel`, then call `complete_codex_security_scan({ scanId, handoffClaimToken? })`. Preserve the actual scan scope and make clear that the report is partial because no source review completed; do not claim the repository is free of vulnerabilities. The usual draft, completion, and error-handling rules below still apply.
