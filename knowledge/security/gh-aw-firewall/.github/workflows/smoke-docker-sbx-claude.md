---
description: Smoke test Docker sandbox with Claude engine
on:
  workflow_dispatch:
  label_command:
    name: test-docker-sbx-claude
    events: [pull_request]
    remove_label: false
  reaction: "eyes"
permissions:
...
safe-outputs:
  ...
  add-labels:
    allowed: [smoke-docker-sbx-claude]
  messages:
    footer: "> 🐳🟣 *Docker sbx + Claude smoke test by [{workflow_name}]({run_url})*"
    run-started: "🐳🟣 [{workflow_name}]({run_url}) is testing Docker sandbox with Claude engine..."
    run-success: "🐳🟣 [{workflow_name}]({run_url}) completed. Docker sbx + Claude smoke test passed. ✅"
...
steps:
  - name: Pre-compute smoke test data
    run: |
      ...
      TEST_FILE="$TEST_DIR/smoke-test-docker-sbx-claude-${GITHUB_RUN_ID}.txt"
      mkdir -p "$TEST_DIR"
      echo "Docker sbx+Claude smoke test passed at $(date)" > "$TEST_FILE"
...
# Smoke Test: Docker Sandbox + Claude Engine
...
- Use the `add_labels` safe-output tool to add the label `smoke-docker-sbx-claude` to the pull request
