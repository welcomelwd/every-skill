...
def cgr_rust_call_edges(
    ...
    caller_path: dict[tuple[str, str], str] = {
        ...
        if props.get(cs.KEY_PATH) and str(props[cs.KEY_PATH]).endswith(ec.RS_SUFFIX)
    }
...
def _edge_repr(edge: CallEdge) -> str:
    return ec.RUST_CALL_EDGE_REPR.format(file=edge[0], name=edge[1])


def score_rust_retrieval(cgr: set[CallEdge], oracle: set[CallEdge]) -> ScoreResult:
    rows: list[ScoreRow] = []
    diff: dict[str, DiffBucket] = {}
    row = _prf(ec.Category.RETRIEVAL.value, ec.RUST_RETRIEVAL_LABEL, cgr, oracle)
    if row is not None:
        rows.append(row)
        diff[ec.RUST_RETRIEVAL_DIFF_PREFIX + ec.RUST_RETRIEVAL_LABEL] = DiffBucket(
