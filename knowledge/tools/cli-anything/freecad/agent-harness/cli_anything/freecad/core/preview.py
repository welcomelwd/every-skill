...
def _preview_base_dir(project_path: Optional[str] = None, root_dir: Optional[str] = None) -> Path:
    ...
    if project_path:
        return Path(project_path).expanduser().resolve().parent / ".cli-anything" / "previews" / "freecad"
    ...
    *,
    ...
    for output in outputs:
        if not os.path.isfile(output["path"]):
            warnings.append(f"missing preview image: {os.path.basename(output['path'])}")
            continue
        artifacts.append(
            artifact_record(
                bundle_dir,
    ...
    manifest = finalize_bundle(
        ...
        generator={
            ...
            "harness_version": HARNESS_VERSION,
            "command": command or f"cli-anything-freecad preview capture --recipe {recipe}",
        },
...
def poll_live_session_once(session_dir: str) -> Dict[str, Any]:
    ...
    try:
        ...
        live_payload = live_start(
            ...
            command=str(payload.get("monitor_command") or f"cli-anything-freecad preview live monitor --session-dir {session_path}"),
