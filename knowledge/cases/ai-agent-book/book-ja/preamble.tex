...
% フォント（lmroman）に無いため、CJK 対応の fallbackfont（Noto Sans CJK JP）で描画する。
\newunicodechar{―}{{\fallbackfont ―}}
\newunicodechar{₀}{{\fallbackfont ₀}}
\newunicodechar{₁}{{\fallbackfont ₁}}
\newunicodechar{₂}{{\fallbackfont ₂}}
...
% 本文の日本語フォント。本書にはコード例や中国語の固有名詞（例: 硅基流动＝SiliconFlow）
...
% ElegantBook には lang=jp が無いため lang=cn を用いる（CJK 組版に必要）。ただし
