...
モデルの訓練コーパスはほぼすべてターン制です——質問の後に回答が続き、ツール呼び出しの後にツール結果が続き、一方が話し終えてから他方が口を開く。そのためモデルが学ぶ方策は、世界が待ってくれることを前提にします。しかし現実の環境は待ちません。考えているあいだにメールが届き、話している途中でユーザーが割り込み、2 回のスクリーンショットのあいだにページはもう変わっており、ロボットアームが手を伸ばす途中でカップが倒される。**本章の四つの節は、この仮定が四つの異なる時間スケールで一つずつ緩められていく過程です。**
...
カスケードシステムを直列実行からストリーミングへ移すとき、最も重要なのは各関数を `async` にすることではなく、**増分結果が無効になり、キャンセルされ得るようにすること**です。
...
終端判定を組み込む場合、ラベルは判断時点で見える情報だけから作る必要がある[^ch6-11]。speak_start/end、interrupt、emotion、laugh、sigh、noise のイベントを文字と一緒に出力すれば、音をすべてテキストに圧縮せず対話状態を扱える。

[^ch6-11]: 終端判定を認識器に組み込む診断と後知恵ラベルの問題については Li, Bojie and Noah Shi. *The Trade-off Was in the Labels: Causal Supervision for Turn-Aware Streaming ASR.* 2026（発表予定）を参照。
...
音声を文字へ変換するカスケードは感情、抑揚、環境音を失うことがある。Omni は一つのモデルで聞き、回答し、話すため情報を保持しやすい。自己カスケードは文字で十分な課題では知覚誤りを直せるが、話速や感情が必要な課題では文字のボトルネックが証拠を消す[^ch6-13]。Omni もターンを前提とするため、数字の途中の無音を終了と誤判定し得る。

[^ch6-13]: カスケードとエンドツーエンドの精度優位が逆転する条件については Li, Bojie and Noah Shi. *The Cascade Gap: When and Why Self-Cascades Help Multimodal Agents.* 2026（発表予定）を参照。
...
MGRD は音響特徴に基づく思考を蒸留し、MPS 双脳構造は計画と表現を並列化する。構想側の思考片を表現側が部分回答と結合して音声化するため、全推論を待たずに話せる。統合型は自然だが共同再訓練が必要で、分離型は背景モデルを交換しやすい。
...
> **実験 6-6 ★★：Fish Audio の制御トークン TTS**
>
> 多参照音声ライブラリを作り、制御なし、単一参照、複数参照を比較する。詳細な 24 参照音声、A/B/C 媒体、受入記録は [chapter6/controllable-tts](../chapter6/controllable-tts/) にある。
...
>
> 経路 B は [`chapter6/computer-use-open-model`](../chapter6/computer-use-open-model/) のサンプルコードを使います。既定ではオープンウェイトの Qwen3-VL 32B Instruct で browser-use を動かし、OpenRouter のホスト API を使うか、`OPEN_MODEL_BASE_URL` をセルフホストの vLLM/SGLang など互換エンドポイントへ向けます。
...
再設計すべきなのは行動インターフェースではなく、**観測インターフェース**です[^ch6-9]。Agent–コンピューター観測インターフェース（AOI）は、連続する環境観測をモデルが扱いやすい離散イベントへ変換します。要点は三つです。**フレーム間キーフレーム捕捉**は、ほぼ変化のない画面を飛ばし、小さなモデルで意味のある変化だけを残します。**音量ゲート付き音声認識**は、音があるときだけ文字起こしします。**フレームをテキストで記述する**ことで、元画像がコンテキストから消えた後も説明が記憶に残り、マルチモーダルな対話履歴を圧縮できます。
