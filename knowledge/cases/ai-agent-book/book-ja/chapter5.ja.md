...
なぜコード生成がこの大役を担えるのでしょうか。それはコードが単なる道具箱の中の一つのツールではなく、一種の**メタ能力**——実行時に動的に新しいツールと能力を創り出せる能力——だからです。本章の後半で、この概念とその 6 つの発揮方向を完全に展開します。
...
なぜ他の 2 つではなく Coding Agent が核心なのでしょうか。それは、ほぼすべての効率的なコンテンツ生成が最終的にコードに落ちるからです。PPT は本質的に OOXML（Office Open XML、マイクロソフトが打ち出したオフィス文書のオープン標準）形式のコードであり、Word 文書や PDF レポートはコードで生成でき、データ分析と可視化は Python スクリプトで完成し、GUI 操作の中で成功したブラウザ操作シーケンスすら再利用可能な RPA（ロボティック・プロセス・オートメーション、Robotic Process Automation）コードとして固定できます（Computer Use 自体は第 6 章、操作シーケンスの固定化メカニズムは第 9 章で詳述します）。Deep Research の検索と情報総合はコード駆動の Web リクエストとパースで実現でき、Computer Use は汎用性こそ高いものの、コストや遅延、安定性の点で同じ操作をコードや API で直接行う場合にはるかに及びません。コード生成は、効率が最も高く、コストが最も低く、再利用性が最も強い能力基盤なのです。
...
モデルによって、このワークフローの省略の仕方は異なる。最初の編集に入る前に、リポジトリ構造、実装、呼び出し元、テストを広く読む Coding モデルもあれば、関係しそうな少数のファイルだけを確認して早期にパッチを当て、コンパイラやテストのフィードバックも調査の一部として使うモデルもある。「いつ情報収集を止めて行動を始めるか」という閾値は、Harness を変えてもモデルに追随して残ることがあり、同じ Harness 内でモデルだけを交換すると変わることもある。したがって、これは単なる Coding 製品のインターフェース様式ではなく、第一義的には**モデルが学習した振る舞い**である。Harness のプロンプト、ツール、予算はこの傾向を強めたり弱めたりできるが、必ずしもその発生源ではない。実験 7-8 では固定 Harness でこの差を測定し、第 8 章では post-training がこのような方策をどのようにパラメータへ書き込み得るかを説明する。
...
この「内から外へ、最終的に自身に戻る」という脈絡に沿って読めば、メタ能力としてのコードの統一された価値をより明瞭に見て取れます。必要に応じて新しいツールを創り出すことは、このメタ能力のさらなる延伸であり、第 9 章で展開します。
...
>
> **技術方案**：`PermissionEmbeddedDataObjects` プロジェクトの実装を使い、PostgreSQL 上に Python オブジェクトストアのミドルウェア層を提供する。データ型が権限規則、アクセスコンテキスト、バリデーター、オブジェクト関係、リアクションを宣言し、すべての読み書きが権限・検証、永続化・参照整合性、制御された非同期リアクションを順に通過する。まず LLM を使わない決定的な採用パイプラインのデモを実行し、任意でモデルに生 SQL と PEDO API への敵対的操作を生成させ、データベース状態を比較する。重要なのは生成ハンドラーに正しい `if` があるかではなく、同じ要求が固定されたデータ層で一貫して受理または拒否されるかどうかである。
>
> **受け入れ基準**：正当な採用パイプラインの更新が成功し、候補者の状態遷移のスキップ、職位の範囲外の給与、テナント間の読み取りがデータ層で拒否されること。権限、検証、テナント分離、参照整合性、リアクションのテストが通過すること。実装は第 5 章の付属プロジェクト `permission-embedded-data-objects` に含まれる。
