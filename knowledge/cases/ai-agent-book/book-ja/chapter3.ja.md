...
第 2 段階は**結果の融合**で、2 路の結果を 1 つの候補プールにまとめます。密検索の類似度（コサイン類似度なら理論上 −1〜1、正規化済みのテキスト埋め込みでは通常 0〜1）と、疎検索の BM25 スコア（0 から数十までさまざま）は尺度も分布も異なり、直接比較できません。一般的な方法は、各路のスコアを正規化して重み付きで加算する方法と、元のスコアを捨てて順位だけを使う逆数順位融合（Reciprocal Rank Fusion, RRF）です。RRF では各文書の総合スコアを Σ 1/(k + rank) とし、平滑化定数 k（通常 60）で上位順位間の差を抑えます。単純で頑健ですが、順位しか使わないため元のスコアが持つ豊かな関連性の信号を失います。
