...
لماذا يعتبر وكيل البرمجة هو الأساس بدلاً من الاثنين الآخرين؟ لأن كل عمليات إنشاء المحتوى الفعالة تقريبًا تتلخص في النهاية في التعليمات البرمجية. إن PPT هو في الأساس رمز بتنسيق OOXML (Office Open XML، معيار Microsoft المفتوح للمستندات المكتبية)؛ يمكن إنشاء مستندات Word وتقارير PDF عبر التعليمات البرمجية؛ يتم تحليل البيانات وتصورها بواسطة البرامج النصية Python؛ حتى سير عمل واجهة المستخدم الرسومية الناجحة يمكن التقاطه ككود RPA (أتمتة العمليات الروبوتية) قابل لإعادة الاستخدام (يتم تناول استخدام الكمبيوتر نفسه في الفصل 6، ويتم تفصيل آلية التقاط تسلسلات العمليات في الفصل 9). يمكن تحقيق البحث وتجميع المعلومات في Deep Research من خلال طلبات الويب والتحليل المستندة إلى التعليمات البرمجية. على الرغم من أن استخدام الكمبيوتر أكثر تنوعًا، إلا أن التعليمات البرمجية المباشرة أو مكالمات API تكون بشكل عام أرخص وأسرع وأكثر موثوقية للعمليات المكافئة. يعد إنشاء التعليمات البرمجية أساس القدرة الأكثر كفاءة والأقل تكلفة والأكثر قابلية لإعادة الاستخدام.
...
فيما يلي وصف **سير العمل الهندسي الموصى به**. ويقدم تطبيقًا مثاليًا لأفضل ممارسات هندسة البرمجيات لوكلاء البرمجة. يعمل وكلاء البرمجة في العالم الحقيقي (مثل Claude Code وOpenClaw) في كثير من الأحيان في حلقة تفاعلية ومتكررة و**يقطعون سير العمل هذا حسب الحاجة** - بالنسبة للمهام البسيطة، فإنهم يتخطون مستند التصميم ولا يمنعون موافقة المستخدم في كل خطوة؛ فقط عندما تكون المهمة معقدة وبعيدة المدى، يقومون بتشغيل كل مرحلة بالكامل.

تختصر النماذج المختلفة سير العمل هذا بطرائق مختلفة. فبعض نماذج البرمجة تقرأ بنية المستودع والتنفيذ ومواضع الاستدعاء والاختبارات على نطاق واسع قبل أول تعديل، بينما تفحص نماذج أخرى بضعة ملفات هي الأرجح صلةً، ثم تنشئ رقعة مبكرة وتتعامل مع ملاحظات المترجم والاختبارات بوصفها جزءًا من الاستقصاء. وقد تظل عتبة تحديد وقت التوقف عن جمع المعلومات وبدء العمل ملازمة للنموذج بعد تغيير الـ Harness، كما قد تتغير عند تبديل النموذج وحده داخل الـ Harness نفسه. لذلك فهذا أولًا **سلوك تعلّمه النموذج**، وليس مجرد أسلوب واجهة في منتج برمجة. ويمكن لموجّهات الـ Harness وأدواته وميزانيته أن تقوّي هذا الميل أو تضعفه، لكنها ليست بالضرورة مصدره. تقيس التجربة 7-8 هذا الفرق في Harness ثابت، ثم يشرح الفصل الثامن كيف يمكن لمرحلة ما بعد التدريب أن تكتب سياسة كهذه في المعلمات.
...
إن اتباع هذا الموضوع - من الداخل إلى الخارج والعودة في النهاية إلى الوكيل نفسه - يجعل رؤية القيمة الموحدة للتعليمات البرمجية كقدرة وصفية أسهل. بناءً على هذا الأساس، يبحث الفصل التاسع في الأدلة التشغيلية التي يجب أن تؤدي إلى التعديل الذاتي وكيف تدخل التعديلات المرشحة إلى إصدار جديد من خلال الاختبار والإصدار والتراجع.
...
>
> **النهج الفني**: استخدام تنفيذ مشروع `PermissionEmbeddedDataObjects` لتوفير طبقة وسيطة لمخزن كائنات Python فوق PostgreSQL. تعلن أنواع البيانات قواعد الصلاحية وسياق الوصول والمدققات والعلاقات والتفاعلات. تمر كل قراءة وكتابة عبر فحوص الصلاحية والتحقق، ثم الحفظ وسلامة المراجع، ثم التفاعلات غير المتزامنة المضبوطة. يبدأ الاختبار بعرض توظيف حتمي بلا LLM، ويمكن اختياريًا جعل النماذج تولّد عمليات عدائية لـ SQL الخام وواجهة PEDO ومقارنة حالة قاعدة البيانات. المقارنة الأساسية هي ما إذا كان الطلب نفسه يُقبل أو يُرفض عند وصوله إلى طبقة البيانات الثابتة.
>
> **معايير القبول**: ينجح تحديث مسار توظيف مشروع، بينما ترفض طبقة البيانات تخطي حالة المرشح والراتب خارج النطاق والقراءة عبر المستأجرين. يجب أن تنجح اختبارات الصلاحيات والتحقق وعزل المستأجر وسلامة المراجع والتفاعلات. يوجد التنفيذ في مشروع الفصل الخامس `permission-embedded-data-objects`.
