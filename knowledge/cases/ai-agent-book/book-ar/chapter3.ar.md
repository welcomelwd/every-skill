...
الحدس لم يتغير، فكلما ندر المصطلح زاد وزنه؛ وإنما تغيّرت طريقة القياس فحسب. صار البسط هو عدد الوثائق التي لا تحتوي على المصطلح، أي $N - \text{DF}(t)$، بدلًا من العدد الكلي $N$، فصارت النسبة تعبّر مباشرة عن عدد المرات التي تفوق فيها الوثائق الخالية من المصطلح تلك التي تحتويه؛ وإضافة 0.5 إلى البسط والمقام تُنعّم النتيجة وتُبقي الصيغة معرَّفة عند الطرفين $\text{DF}(t) = 0$ و$\text{DF}(t) = N$. والثمن أن المصطلح الوارد في أكثر من نصف الوثائق ($\text{DF}(t) > N/2$) يحصل على وزن سالب، ولذلك تضع التطبيقات العملية حدًّا أدنى له عادةً. وهذه الصيغة مستمدة من نموذج الاسترجاع الاحتمالي، وتُعرف في الأدبيات بوزن Robertson–Spärck Jones.
...
المرحلة الثانية هي **دمج النتائج** في مجموعة مرشحة موحدة. ولا يمكن مقارنة الدرجات مباشرة، لأن درجات التشابه في الاسترجاع الكثيف ودرجات BM25 تعمل على مقاييس وتوزيعات مختلفة. من الأساليب الشائعة تطبيع درجات كل مسار ثم جمعها بأوزان، أو استخدام دمج الرتب المتبادل (RRF)، الذي يعتمد على الرتب فقط: الدرجة = Σ 1/(k + الرتبة)، وعادةً ما تكون k مساوية لـ 60. يتميز RRF بالبساطة والمتانة، لكنه يهمل بعض إشارات الصلة الموجودة في الدرجات الأصلية.
...
**الحالة 2: مشكلة حدود الأهلية في خصم Xfinity.** هذه المرة قاعدة المعرفة أرشيف لتذاكر دعم العملاء: بضع مئات من التذاكر، تسجّل كل واحدة منها نتيجة معالجة واقعية واحدة — تمت الموافقة على طلب المحارب القديم جون، وحصلت الدكتورة سارة على الخصم، وأُبلغ المعلم مايك بأنه غير مؤهل، وهكذا. كل تذكرة تدوّن خلاصة حالة فردية واحدة فقط، ولا توجد تذكرة واحدة تدوّن نطاق الأهلية نفسه. وعندما تسأل ممرضة "هل أنا مؤهلة؟" تتراكم عدة عقبات. أولًا **انحياز الجار الأقرب**: كلمة "ممرضة" أقرب دلاليًا إلى "طبيبة"، فتتصدر تذكرة سارة الترتيب، ويستنتج النموذج تبعًا لذلك أن الممرضات مؤهلات أيضًا؛ ولو صادف أن جاءت تذكرة مايك في المقدمة لحصل السؤال نفسه على الإجابة المعاكسة. **الإجابة تحددها التذكرة الأقرب إلى الاستعلام، لا السياسة نفسها.** ثانيًا **غياب دلالة الحدود**، وهي عقبة لا تحلها زيادة k: فعبارة من نوع "فقط ...، وكل المهن الأخرى غير مؤهلة" تحمل تسويرًا كليًا ونفيًا، ولا توجد في أي تذكرة منفردة، بل في انغلاق (closure) المدونة بأكملها فحسب. الأرشيف أصلًا لا يجيب عن سؤال "هل تُحتسب الممرضة"، ومن ثم فإن إجبار النموذج على استقراء قاعدة كلية من حفنة حالات فردية ينتج استنتاجًا لم يكن صحيحًا من الأساس. ثالثًا **غياب إشارة الاكتمال**: لا سبيل للنموذج ليعرف هل رأى القاعدة كاملة أم لا، فلا يستفسر، بل يجيب بثقة اعتمادًا على التذاكر القليلة التي بين يديه. والحل يعود مجددًا إلى مرحلة الفهرسة: اقرأ أرشيف التذاكر كاملًا دون اتصال، واتخذ سياسة الأهلية الرسمية مرجعًا (بدلًا من الاستقراء من الحالات القليلة المسترجَعة، وهو بالضبط تلوث المعرفة الذي يحذَّر منه لاحقًا)، ثم استخلص بطاقة قاعدة واحدة: "خصومات Xfinity تسري على العسكريين في الخدمة الفعلية والمحاربين القدامى، وعلى العاملين الصحيين المرخَّصين بمن فيهم الممرضات؛ والمهن الأخرى كالتعليم غير مؤهلة؛ والمهن غير المدرجة تتطلب مراجعة بشرية." وبعد كتابة الحد والحالة الاحتياطية معًا، تكفي عملية استرجاع واحدة للحصول على القاعدة الكاملة أيًّا كانت المهنة المسؤول عنها — فلم يعد على النموذج أن يستقرئ، بل أن يطابق فحسب.
