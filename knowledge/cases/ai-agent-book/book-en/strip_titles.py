...
According to academic conventions, the figure itself does not contain a title
(the title is written in the main text). This script conservatively handles the
checked-in SVG files under `images/`.

Judgment rule (conservative):
- The first occurrence of a <text> element, if it simultaneously satisfies: font-size>=20 and y<60 and text-anchor=middle,
  is considered a figure title—delete that element, and shift the viewBox top edge and height downward/shrink by CROP_PX.
- Otherwise, the file is not modified.

Usage: python3 strip_titles.py [--dry-run]
"""
