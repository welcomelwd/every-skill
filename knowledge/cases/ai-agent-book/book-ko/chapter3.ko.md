...
충돌 탐지에는 버전 관리 방식을 사용합니다. 과거 버전은 유지하고 최신 버전을 표시합니다. 현재 주소 같은 정보는 최신 버전만 남기고, 경력처럼 이력이 중요한 정보는 전체 기록을 보존합니다.
...
두 예시의 패턴은 같습니다. **관련 조각 검색 → 컨텍스트에 주입 → LLM이 컨텍스트를 바탕으로 답변 생성**입니다. RAG의 핵심 가치는 모델을 다시 학습하지 않고도 LLM이 학습 중에 보지 못한 지식, 예컨대 Wikipedia의 최신 내용이나 기업 내부 문서를 활용하게 해 주는 데 있습니다.
...
**학습형 희소 검색.** 이 장에서는 학습이 필요 없고 투명하게 재현할 수 있어 희소 검색의 원리를 설명하기에 가장 적합한 고전적 BM25를 대표 사례로 사용합니다. 그러나 희소 검색 자체도 이미 “학습형” 단계에 들어섰습니다. SPLADE 같은 모델과 BGE-M3의 희소 출력 분기는 신경망으로 각 단어의 가중치를 정합니다. BM25처럼 단어 빈도와 문서 빈도만으로 점수를 매기지 않고, 모델이 “이 텍스트에서 이 단어가 얼마나 중요한가”를 판단합니다. 원문에 없지만 의미상 관련 있는 단어에 0이 아닌 가중치를 부여하는 용어 확장도 가능합니다. 결과는 여전히 대부분 차원의 값이 0인 희소 벡터이므로 어휘적 해석 가능성과 완전 일치 능력을 유지하면서, 신경망의 의미 일반화 능력 일부를 얻습니다. 희소 검색과 밀집 검색이 만나는 지점이라고 생각할 수 있습니다.
...
둘째는 **결과 융합**입니다. 코사인 유사도와 BM25 점수는 척도와 분포가 달라 직접 비교할 수 없습니다. 보통 각 경로를 정규화해 가중합하거나, 원래 점수를 버리고 순위만 사용하는 Reciprocal Rank Fusion(RRF)을 적용합니다. RRF 점수는 Σ 1/(k + rank)이며 k는 흔히 60입니다. 단순하고 견고하지만 원래 점수의 풍부한 관련성 신호를 잃습니다.
...
비유하자면 이중 인코더는 이력서를 훑어 1차로 선별하는 채용 담당자이고, 교차 인코더는 각 후보와 깊이 대화하는 면접관입니다. 전자는 미리 추출한 특성으로 대규모 후보를 거르고, 후자는 질의와 각 후보 문서가 “직접 마주 보게” 하여 단어 단위로 평가합니다. 재순위 모델이 사용하는 “Cross-Encoder” 아키텍처는 검색 단계의 “Bi-Encoder”와 뚜렷이 다릅니다. **Bi-Encoder**는 질의와 문서에 독립된 벡터를 생성하고 벡터 연산으로 유사도를 계산합니다. 매우 빠르지만 깊은 일치 관계를 포착하지 못하므로 대규모 데이터의 1차 선별에 적합합니다. **Cross-Encoder**는 **질의와 후보 문서를 하나의 텍스트로 이어 붙여** 모델에 입력합니다. 모델은 단어 단위로 비교하여 종합적인 관련성 점수[^ch3-cross-encoder]를 출력합니다. 속도는 훨씬 느리지만 관련성을 더 정확히 판단합니다. 널리 사용하는 재순위 모델인 [BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3)도 이 아키텍처를 채택합니다.

이 “공동 어텐션” 메커니즘 덕분에 교차 인코더는 이중 인코더가 감지하지 못하는 미묘한 의미 연관성을 포착할 수 있으며, 어떤 단일 검색 방식보다 훨씬 정확한 최종 순위를 만듭니다.

[^ch3-cross-encoder]: BERT 계열 모델을 구현할 때는 이어 붙인 입력을 특수 토큰으로 구분합니다(예: `[CLS] query text [SEP] document text [SEP]`. `[CLS]`는 시퀀스의 시작을, `[SEP]`는 경계를 나타냄). 검색 과정을 이해하는 데 꼭 필요하지 않은 내부 구현 세부 사항입니다.
...
업계 보고서에서는 “검색 실패율”도 흔히 언급합니다. 예를 들어 이 장 뒤에서 인용하는 Anthropic 데이터에서 검색 실패율은 상위 20개 검색 결과에 올바른 정보가 나타나지 않은 질의의 비율, 즉 사실상 1 − recall@20을 뜻합니다. 이런 수치를 접하면 비교하기 전에 어느 지표에 해당하는지, k는 얼마인지 확인해야 합니다.
