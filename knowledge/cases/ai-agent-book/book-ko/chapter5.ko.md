...
코드 생성은 어떻게 이처럼 중요한 역할을 감당할 수 있을까요? 코드 생성은 도구 상자에 든 여러 도구 중 하나가 아니라, 런타임에 새로운 도구와 기능을 동적으로 만들어 내는 **메타 역량**이기 때문입니다. 이 장의 후반부에서는 이 개념과 그 여섯 가지 적용 방향을 자세히 설명합니다.
...
다음은 **권장 엔지니어링 워크플로**입니다. 소프트웨어 엔지니어링 모범 사례를 코딩 에이전트에 이상적으로 적용한 모습을 제시합니다. 현실의 코딩 에이전트(Claude Code, OpenClaw 등)는 대개 반응적이고 반복적인 루프로 작업하며 **필요에 따라 이 워크플로를 축소합니다**. 단순한 작업에서는 설계 문서를 생략하고 각 단계에서 사용자 승인을 기다리지 않습니다. 작업이 복잡하고 광범위할 때만 모든 단계를 온전히 수행합니다.

모델마다 이 워크플로를 줄이는 방식은 다르다. 어떤 Coding 모델은 첫 편집 전에 저장소 구조, 구현, 호출부, 테스트를 폭넓게 읽는다. 다른 모델은 관련 가능성이 높은 소수의 파일만 살펴본 뒤 일찍 패치를 만들고, 컴파일러와 테스트 피드백까지 조사 과정의 일부로 사용한다. ‘언제 정보 수집을 멈추고 행동을 시작할지’를 정하는 이 임계값은 Harness가 바뀐 뒤에도 모델을 따라 유지될 수 있고, 같은 Harness 안에서 모델만 교체하면 달라질 수도 있다. 따라서 이는 단순한 Coding 제품의 인터페이스 스타일이 아니라 우선적으로 **모델이 학습한 행동**이다. Harness의 프롬프트, 도구, 예산은 이 성향을 강화하거나 억제할 수 있지만 반드시 그 원천인 것은 아니다. 실험 7-8은 고정된 Harness에서 이 차이를 측정하고, 8장에서는 post-training이 이런 정책을 어떻게 파라미터에 기록할 수 있는지 설명한다.
...
- **하이브리드 검색**(3장에서 이 기술 스택을 자세히 설명합니다): 벡터 임베딩(밀집 임베딩)은 표현은 다르지만 의미가 유사한 코드를 찾는 데 강합니다(예: "사용자 신원 확인"을 검색하여 `check_credentials`라는 함수를 찾을 수 있습니다). 반면 키워드 일치(BM25, 용어 빈도와 문서 길이에 기반한 고전적인 검색 알고리즘)는 함수와 변수 이름을 정확히 일치시키는 데 강합니다. 둘을 병렬로 실행하고 재순위화 모델(후보 결과의 관련성을 세밀하게 정렬하는 교차 인코더)이 결과를 병합·정렬하여 상호 보완적으로 검색합니다.

의미 기반 검색은 익숙하지 않은 코드베이스에서 "데이터베이스와 상호작용" 또는 "사용자 입력 검증 처리"와 관련된 코드를 찾는 식의 탐색 작업에 특히 적합합니다.

하지만 의미 기반 검색을 위해 임베딩 인덱스를 구축할 가치가 있는지를 두고 업계에서는 뚜렷한 논쟁이 있습니다. Claude Code 같은 터미널 기반 에이전트는 의도적으로 **임베딩 인덱스를 만들지 않고**, 에이전트형 grep + glob만으로 그때그때 검색합니다. 코드가 발전하면서 오래되는 인덱스를 유지할 필요가 없고, 인덱싱 인프라 전체를 없앨 수 있으며, 코드 임베딩을 제3자 서비스로 전송할 위험도 피할 수 있습니다. Cursor 같은 IDE 기반 도구는 반대 접근법을 취합니다. **파일 간 의미 재현율**을 얻기 위해 인덱스 구축 비용을 감수하고, 임베딩 인덱스로 대규모 코드베이스에서 의미는 관련 있지만 표현이 다른 코드 조각을 빠르게 찾습니다. 두 경로의 절충은 본질적으로 "인프라와 데이터 이그레스 비용"과 "파일 간 의미 재현율의 이점"을 저울질하는 문제입니다.

**심벌 수준 정의·참조 검색**: IDE의 "정의로 이동"과 "모든 참조 찾기" 기능(LSP, 즉 편집기와 언어 분석 엔진 사이의 통신을 위한 표준 프로토콜인 Language Server Protocol로 제공)을 사용해 심벌 정의와 참조를 구분합니다. 예를 들어 42번째 줄의 `authenticate`는 함수 정의이고 189번째 줄의 출현은 호출이라고 식별하지만, 텍스트 검색은 해당 문자열이 포함된 모든 줄만 찾을 수 있습니다. 이는 코드 리팩터링에서 특히 중요합니다. 함수 이름을 바꿀 때 텍스트 검색에만 의존할 수 없습니다. 함수 이름이 주석이나 문자열에도 나타날 수 있기 때문입니다. 심벌 검색을 사용해 정의와 실제 호출 위치를 모두 정확히 찾아야 합니다.
...
안에서 밖으로 나아가 결국 에이전트 자체로 돌아오는 이 흐름을 따라가면 메타 역량으로서 코드가 지닌 통합된 가치를 더 쉽게 이해할 수 있습니다. 9장에서는 이 토대 위에서 어떤 운영 증거가 자기 수정을 촉발해야 하는지, 수정 후보가 테스트·릴리스·롤백을 거쳐 새 버전에 어떻게 반영되는지 살펴봅니다.
...
>
> **기술적 접근법**: `PermissionEmbeddedDataObjects` 프로젝트 구현을 사용해 PostgreSQL 위에 Python 객체 저장소 미들웨어 계층을 제공합니다. 데이터 유형이 권한 규칙, 접근 컨텍스트, 검증기, 객체 관계, 반응을 선언하고, 모든 읽기·쓰기가 권한 및 검증 검사, 영속화와 참조 무결성 처리, 통제된 비동기 반응을 통과하게 합니다. 먼저 LLM 없이 결정적인 채용 파이프라인 데모를 실행하고, 선택적으로 모델에게 원시 SQL과 PEDO API를 공격하는 작업을 생성하게 한 뒤 데이터베이스 상태를 비교합니다. 핵심 비교 대상은 생성된 핸들러에 올바른 `if`가 있는지가 아니라 동일한 요청이 고정된 데이터 계층에 도달했을 때 일관되게 승인 또는 거부되는지입니다.
>
> **승인 기준**: 유효한 채용 파이프라인 업데이트가 성공해야 하며, 후보자 상태 전이 건너뛰기, 직무 범위를 벗어난 급여 쓰기, 테넌트 간 읽기는 데이터 계층에서 거부되어야 합니다. 권한, 검증, 테넌트 격리, 참조 무결성, 반응 테스트가 통과해야 합니다. 구현은 5장 부록 프로젝트 `permission-embedded-data-objects`에 포함됩니다.
