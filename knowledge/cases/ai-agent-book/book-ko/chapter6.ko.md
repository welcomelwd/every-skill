...
모델의 훈련 말뭉치는 거의 전부 턴제입니다. 질문 뒤에 답이 오고, 도구 호출 뒤에 도구 결과가 오며, 한쪽이 말을 마쳐야 상대가 입을 엽니다. 그래서 모델이 배운 정책은 세계가 자신을 기다려 준다고 전제합니다. 그러나 현실의 환경은 기다리지 않습니다. 생각하는 사이에 메일이 도착하고, 말하는 도중에 사용자가 끼어들며, 두 번의 스크린샷 사이에 페이지는 이미 바뀌어 있고, 로봇 팔이 손을 뻗는 도중에 컵이 넘어집니다. **이 장의 네 절은 그 가정이 네 가지 시간 척도에서 하나씩 완화되는 과정입니다.**
...
캐스케이드 시스템을 직렬 실행에서 스트리밍으로 전환할 때 가장 중요한 것은 모든 함수를 `async`로 바꾸는 일이 아니라 **증분 결과가 무효화되고 취소될 수 있게 하는 것**이다.
...
종단 판단을 인식기에 넣을 때 라벨은 결정 시점에 보이는 정보만 사용해야 한다. 사후 정보를 이용하면 온라인에서 재현할 수 없는 판단이 된다[^ch6-11]. speak_start/end, interrupt, emotion, laugh, sigh, noise 사건 표식은 모든 소리를 텍스트로 압축하지 않고도 변화를 보존한다.

[^ch6-11]: 종단 판단과 사후 라벨 문제는 Bojie Li and Noah Shi. *The Trade-off Was in the Labels: Causal Supervision for Turn-Aware Streaming ASR.* 2026 (forthcoming)을 참고하라.
...
캐스케이드는 오디오를 텍스트로 바꾸는 경계에서 감정·억양·환경음을 잃을 수 있다. Omni는 하나의 모델로 듣고 답하고 말해 이를 보존할 가능성이 있지만 학습·디버깅·교체 비용이 높다. 엔드투엔드의 이점은 지연과 비텍스트 정보이지 반드시 정확도는 아니다. self-cascade는 텍스트가 충분한 과제에서는 오류를 고치지만 말속도·감정이 필요한 과제에서는 텍스트 병목이 증거를 지운다[^ch6-13]. Omni도 턴을 가정해 숫자 중간의 침묵을 끝으로 오인할 수 있다.

[^ch6-13]: 조건별 캐스케이드/엔드투엔드 정확도 측정은 Bojie Li and Noah Shi. *The Cascade Gap: When and Why Self-Cascades Help Multimodal Agents.* 2026 (forthcoming)을 참고하라.
...
MGRD는 음향 특징에 사고를 정박하고 MPS 듀얼 브레인은 계획과 표현을 병렬화한다. 계획 뇌가 사고 조각을 내보내고 표현 뇌가 부분 답변과 합쳐 즉시 음성화하므로 전체 추론을 기다리지 않는다. 통합형은 자연스럽지만 공동 재훈련이 필요하고 분리형은 백그라운드 모델을 바꾸기 쉽다.
...
> **실험 6-6 ★★: Fish Audio의 제어 토큰 TTS**
>
> 다중 참조 음성 라이브러리를 만들고 제어 없음·단일 참조·다중 참조를 비교한다. 표식에서 감정·말속도·스타일을 선택한다. 전체 기록은 [chapter6/controllable-tts](../chapter6/controllable-tts/)에 있다.
...
> **실험 6-7 ★: 컴퓨터 사용 실행(Anthropic 참조 경로 또는 오픈 모델 경로)**
>
...
>
> 경로 B는 [`chapter6/computer-use-open-model`](../chapter6/computer-use-open-model/)의 예제 코드를 사용합니다. 기본적으로 오픈 웨이트 Qwen3-VL 32B Instruct로 browser-use를 구동하며, OpenRouter 호스팅 API를 쓰거나 `OPEN_MODEL_BASE_URL`을 자체 호스팅 vLLM/SGLang 또는 다른 호환 엔드포인트로 지정할 수 있습니다.
...
재설계해야 할 것은 행동 인터페이스가 아니라 **관찰 인터페이스**입니다[^ch6-9]. Agent–컴퓨터 관찰 인터페이스(AOI)는 연속적인 환경 관찰을 모델이 처리하기 쉬운 이산 이벤트로 변환합니다. 핵심 기술은 세 가지입니다. **프레임 사이 핵심 프레임 포착**은 거의 변하지 않은 화면을 건너뛰고 작은 모델로 의미 있는 변화만 남깁니다. **음량 게이트 음성 전사**는 소리가 있을 때만 인식을 호출합니다. **프레임을 텍스트로 서술하기**는 원본 이미지가 컨텍스트에서 제거된 뒤에도 설명을 기억에 남겨 멀티모달 상호작용 기록을 압축합니다.
