-- crossref.lua — internal cross-reference links for the Spanish edition.
--
-- Keeps the existing manual numbering (Figura N-M, Capítulo N) but turns every
...
--
-- Spanish references span two inline elements: Str("Figura") Space Str("2-6")
-- or Str("Capítulo") Space Str("4").
...
-- digit, or hyphen (punctuation, em dashes, closing quotes/parens…).
...
-- Split "…Figura" / "…Capítulo" tokens: the keyword may carry glued leading
-- punctuation, ASCII or multi-byte (e.g. "(Figura", "fundamentos—Capítulo").
...
return {
  {
    ...
    Figure = function(el)
      ...
      local n, m = cap:match('[Ff]igura%s*(%d+)%-(%d+)')
      if n and m then
    ...
    Inlines = function(inlines)
      ...
      while i <= n do
        ...
        if el.t == 'Str' and i + 2 <= n
          ...
          if pre then kw_found = 'Figura' end
          if not pre then
            pre = split_kw(el.text, 'figura')
            if pre then kw_found = 'figura' end
          end
          local kind = 'fig'
          if not pre then
            ...
            if pre then kw_found = 'Capítulo' end
            if not pre then
              pre = split_kw(el.text, 'capítulo')
              if pre then kw_found = 'capítulo' end
            end
            ...
            if kind == 'fig' then
              ...
              if a and ok_suffix(suffix) then
                ...
                out:insert(pandoc.RawInline('latex',
                  '\\crossreflink{' .. fig_label(a, b) .. '}{' .. kw_found .. ' ' .. a .. '-' .. b .. '}'))
              ...
              if a and ok_suffix(suffix) then
                if pre ~= '' then out:insert(pandoc.Str(pre)) end
                out:insert(pandoc.RawInline('latex',
                  '\\crossreflink{' .. chap_label(a) .. '}{' .. kw_found .. ' ' .. a .. '}'))
                if suffix ~= '' then out:insert(pandoc.Str(suffix)) end
                linked = true
              end
            end
