...
מדוע יצירת קוד יכולה לשאת משקל זה? משום שהיא אינה רק כלי אחד בארגז הכלים, אלא **מטא‑יכולת** — היכולת ליצור כלים ויכולות חדשים באופן דינמי בזמן ריצה. המחצית השנייה של פרק זה מפתחת מושג זה במלואו, יחד עם ששת הכיוונים שבהם הוא חל.
...
מוצרי סוכן לשימוש כללי כגון Manus משלבים שלוש יכולות מרכזיות — Deep Research,‏ Computer Use ותכנות — במערכת אחת, ובכך מחזקים תובנה שהתאמתה שוב ושוב בפועל: **סוכן קוד בתוספת מערכת קבצים הוא הבסיס הטכני היסודי ביותר לסוכנים כלליים פתוחים.** פרויקט הקוד הפתוח OpenClaw נוקט גישה דומה, ומדגים את אותה פרדיגמה ארכיטקטונית בגלוי.

מדוע סוכן הקוד הוא הליבה ולא שני האחרים? משום שכמעט כל יצירת תוכן יעילה מסתכמת בסופו של דבר בקוד. מצגת PPT היא במהותה קוד בפורמט OOXML‏ (Office Open XML, התקן הפתוח של מיקרוסופט למסמכי משרד); מסמכי Word ודוחות PDF ניתנים ליצירה באמצעות קוד; ניתוח נתונים וויזואליזציה נעשים על ידי סקריפטי Python; אפילו זרימות עבודה מוצלחות ב‑GUI ניתנות ללכידה כקוד RPA‏ (Robotic Process Automation) בר‑שימוש חוזר (Computer Use עצמו מכוסה בפרק 6, ומנגנון לכידת רצפי פעולות מפורט בפרק 9). החיפוש וסינתזת המידע של Deep Research ניתנים להשגה באמצעות בקשות רשת וניתוח מונחי קוד. אף ש‑Computer Use רב‑תכליתי יותר, קוד ישיר או קריאות API בדרך כלל זולים, מהירים ואמינים יותר לפעולות שקולות. יצירת קוד היא בסיס היכולות היעיל ביותר, בעל העלות הנמוכה ביותר והנתון ביותר לשימוש חוזר.
...
**גבול התחולה: לאילו סוכנים תכנות הוא ארכיטקטורת הליבה.** המסקנה ש"סוכן הקוד הוא הליבה של סוכן לשימוש כללי" חלה בעיקר על **סוכנים לשימוש כללי המכוונים למשימות פתוחות** — תרחישים כגון מחקר עמוק, יצירת תוכן ועיבוד נתונים, שבהם גבולות המשימה בלתי ודאיים וצורות התוצרים מגוונות. בתרחישים אלה, בלתי אפשרי למנות מראש את כל הכלים הנדרשים; יצירת קוד, כמטא‑יכולת, מספקת את המסלול הכלכלי ביותר להרחבה דינמית של גבולות היכולת, ובכך הופכת אותה לליבת הארכיטקטורה. לעומת זאת, סוכני שירות לקוחות בתחום אנכי ועוזרי קול פועלים במרחבי משימה סגורים יחסית, כשארכיטקטורות הליבה שלהם בנויות סביב תהליכים עסקיים קבועים, כלים תחומיים ואסטרטגיות דיאלוג; שם, קוד הוא כלי בארגז הכלים ולא צומת ארכיטקטוני (בדוגמת τ‑bench בהמשך פרק זה — מדד המדמה תרחישי שירות לקוחות — קוד ממלא בדיוק את תפקיד כלי אימות המדיניות). עם זאת, גם באחרונים, תכנות הוא יכולת יסוד שאין בלתה: חישוב מדויק, עיבוד נתונים ואימות כללים תלויים בו כולם — הדבר מהדהד את הטענה בסעיף הקודם, "תכנות כיכולת יסוד של סוכן": האם תכנות הוא ארכיטקטורת הליבה תלוי בתרחיש, אך החזקת יכולת תכנות היא קו בסיס משותף לכל הסוכנים.
...
להלן תיאור של **זרימת עבודה הנדסית מומלצת**. היא מציגה יישום אידיאלי של שיטות עבודה מומלצות בהנדסת תוכנה על סוכני קוד. סוכני קוד מהעולם האמיתי (כגון Claude Code,‏ OpenClaw) עובדים לעיתים קרובות יותר בלולאה תגובתית ואיטרטיבית ו**גוזמים זרימת עבודה זו לפי הצורך** — למשימות פשוטות הם מדלגים על מסמך העיצוב ואינם נעצרים לאישור המשתמש בכל שלב; רק כאשר משימה מורכבת ורחבת השלכות הם מריצים כל שלב במלואו.

מודלים שונים גוזמים זרימת עבודה זו בדרכים שונות. מודלי תכנות מסוימים קוראים את מבנה המאגר, המימוש, הקוראים והבדיקות בהרחבה לפני העריכה הראשונה. אחרים בוחנים רק את הקבצים הבודדים הסבירים ביותר להיות רלוונטיים, מבצעים תיקון מוקדם, ומתייחסים למשוב הקומפיילר והבדיקות כחלק מהחקירה. סף ההחלטה הזה — מתי להפסיק לאסוף מידע ולהתחיל לפעול — יכול להמשיך ללוות את המודל גם לאחר שה‑Harness משתנה, ויכול להשתנות כשמחליפים את המודל בתוך אותו Harness. הוא אפוא בראש ובראשונה **התנהגות מודל נלמדת**, ולא רק סגנון הממשק של מוצר תכנות. פרומפטים, כלים ותקציבים ב‑Harness עדיין יכולים להגביר או להחליש אותו, אך אינם חייבים להיות מקורו. ניסוי 7‑8 מודד הבדל זה ב‑Harness קבוע; פרק 8 מסביר לאחר מכן כיצד אימון־על עשוי לכתוב מדיניות כזו לתוך הפרמטרים.
...
ערמומי יותר ממפסק בנקודה בודדת הוא **סחרור המוות**: לוגיקה המופעלת במסלול השגיאה קוראת בעצמה ל‑LLM, נכשלת שוב, ומתפשטת. סחרור אמיתי אחד: הסוכן נעצר על שגיאת גלישת הקשר, המפעילה stop hook (לוגיקת ניקוי הרצה אוטומטית כשהסוכן מסתיים) ה"מבצע קומיט לקוד ביציאה", ה‑hook קורא ל‑LLM כדי לכתוב הודעת קומיט, ההקשר גולש שוב, וה‑hook מופעל פעם נוספת. ההגנה מגיעה בשני חלקים: השביתו את כל תופעות הלוואי הקוראות למודל במסלול השגיאה (מוטב לאבד תכונת עזר פעם אחת, כגון חילוץ זיכרון אוטומטי), והשתמשו במונה עומק רקורסיה כדי לזהות ולשבור כל סחרור שנותר. לבסוף, מעל כל המנגנונים האוטומטיים יושבים תנאי סיום והסלמה גלובליים: מספר תורות מרבי, תקרת תקציב לסשן, והסלמה להתערבות אנושית כשכשלים רצופים חורגים מהסף שלהם (מפסק הדחיות של פרק 4 הוא דוגמה אחת).

בחזרה לשאלת המחשבה של פרק 1, סוכן יכול להילכד בלולאה לא רק בשל תוצאות כלים חסרות אלא גם בשל שגיאות כלים זהות חוזרות, קריאות הזויות, דחיסת הקשר המאבדת מצב מרכזי, או משימה בלתי פתירה. הזיהוי נשען על "סיווג שגיאות + זיהוי דפוסים", ההתאוששות על "הסלמה מדורגת", והסיום על "מפסקים + תקרות גלובליות + הסלמה אנושית" — יחד אלה הם התשובה המלאה של ה‑Harness ל"ייתכן שהסוכן ירוץ לנצח". מה שמנגנונים אלה פותרים אינו "יכולת מודל בלתי מספקת" אלא "עמידות המערכת בתנאי גבול": מודלים ימשיכו להתחזק, אך רשתות יתנתקו, תהליכים ייתקעו, ומשתמשים יעשו דברים בלתי צפויים. באופן יסודי יותר, **אמינותו של סוכן אינה נקבעת לפי האם הוא עושה טעויות, אלא לפי האם לכל מחלקת שגיאות יש מסלול זיהוי, התאוששות וסיום מתאים**.
...
- **אחזור היברידי** (פרק 3 מפרט מחסנית טכנולוגיה זו): שיכוני וקטורים (שיכונים צפופים) מצטיינים במציאת קוד דומה סמנטית בניסוח שונה (למשל, חיפוש "אמת זהות משתמש" יכול למצוא פונקציה בשם `check_credentials`), בעוד שהתאמת מילות מפתח (BM25, אלגוריתם אחזור קלאסי המבוסס על תדירות מונחים ואורך מסמך) מצטיינת בהתאמה מדויקת של שמות פונקציות ומשתנים. השניים רצים במקביל, והתוצאות ממוזגות וממוינות על ידי מדרג מחדש (מקודד צולב המבצע דירוג רלוונטיות ברמת פירוט עדינה על תוצאות מועמדות), ומספקים כיסוי משלים.
...
עם זאת, קיים ויכוח ברור בתעשייה האם כדאי לבנות אינדקסי שיכונים לחיפוש סמנטי. סוכנים מבוססי טרמינל כגון Claude Code **אינם בונים בכוונה אינדקסי שיכונים**, ונשענים אך ורק על grep + glob סוכניים לאחזור בזמן אמת — הדבר נמנע מתחזוקת אינדקסים המתיישנים ככל שהקוד מתפתח, מבטל את תשתית האינדוקס כולה, ונמנע מהסיכון של שליחת שיכוני קוד לשירותי צד שלישי. כלים מבוססי IDE כגון Cursor נוקטים בגישה ההפוכה: הם מוכנים לשלם את מחיר בניית האינדקסים תמורת **היזכרות סמנטית חוצת‑קבצים**, ומשתמשים באינדקסי שיכונים כדי למצוא במהירות קטעים קשורים סמנטית אך מנוסחים אחרת בבסיסי קוד גדולים. הפשרה בין שני המסלולים מסתכמת במהותה בשקילת "עלות התשתית ויציאת הנתונים" מול "התועלת של היזכרות סמנטית חוצת‑קבצים".

**חיפוש הגדרות והפניות ברמת הסמלים**: שיטה זו משתמשת ביכולות "מעבר להגדרה" ו"מצא את כל ההפניות" של ה‑IDE (המסופקות באמצעות LSP, או Language Server Protocol — פרוטוקול תקני לתקשורת בין עורכים למנועי ניתוח שפה) כדי להבחין בין הגדרות סמלים להפניות אליהם — לדוגמה, הוא מזהה את `authenticate` בשורה 42 כהגדרת פונקציה ואת המופע בשורה 189 כקריאה, בעוד שחיפוש טקסט יכול למצוא רק את כל השורות המכילות את אותה מחרוזת. הדבר קריטי במיוחד לריפקטורינג קוד — בעת שינוי שם פונקציה, אינכם יכולים להסתמך על חיפוש טקסט בלבד (שם הפונקציה עשוי להופיע בהערות או במחרוזות); עליכם להשתמש בחיפוש סמלים כדי לאתר בדייקנות את ההגדרה ואת כל אתרי הקריאה בפועל.
...
הדבר מסביר גם מדוע סוכנים מסחריים בקוד סגור (כגון Claude Cowork (הסוכן לשימוש כללי של Anthropic לעבודת ידע, העושה שימוש חוזר בארכיטקטורה הסוכנית של Claude Code, ומסוגל לקרוא ולכתוב קבצים מקומיים ולהשלים משימות רב‑שלביות על פני יישומי משרד מרובים)) בחרו באסטרטגיות הרשאה שמרניות — לא משום שהטכנולוגיה אינה מספקת, אלא משום שסיכוני האבטחה גבוהים מדי. נגד הזרקת פרומפט, סינון קלט בלבד כמעט אינו מסייע. המטרה אינה לזהות כל התקפה, אלא להבטיח שסוכן שהוזרק לעולם לא יקבל את ההזדמנות להוציא פעולה מסוכנת אל הפועל. מערכת ההגנה בוססה שכבה אחר שכבה בשני הפרקים הקודמים: **הגנת שכבת ההקשר** — סימון מקורות תוכן חיצוניים, בידוד תפקידים מובנה, חיטוי קלט — ראו את סעיף הזרקת הפרומפט בפרק 2; **הגנת שכבת הביצוע** — סקירת Sidecar עצמאית, אדם בלולאה, הרשאה מינימלית והפרדת סמכויות — ראו פרק 4. מכיוון שסוכן אינו יכול לקבוע באמינות האם ההקשר שלו עצמו נפרץ, פעולות קריטיות חייבות להיסקר על ידי מנגנונים מחוץ לאותו הקשר. עיקרון זה עובר לאורך שני הפרקים. סעיף זה מוסיף רק שלוש הגנות ספציפיות הייחודיות לסוכני קוד: זו בדיוק חלוקת העבודה בין שלוש השכבות מפרק 1: שכבת ההקשר מורידה את שיעור ההצלחה, בעוד ששכבות הביצוע והנתונים מספקות את הערובה.
...
**בידוד כרשת ביטחון: בחירות הנדסיות לארגז החול של הרצת הקוד.** ארגז חול אינו מתג; הוא סדרת החלטות הנדסיות. פרק 4 כבר הסביר מדוע בידוד נחוץ, התווה את הספקטרום התלת‑שכבתי של מנגנוני בידוד (בידוד ברמת תהליך, מכולות ו‑microVM), וסיפק את כלל הבחירה הזה: בידוד ברמת תהליך למכונות מקומיות אישיות, מכולות לסביבות ענן חד‑דייריות, ו‑microVM או gVisor לסביבות רב‑דייריות או לקוד בלתי מהימן. במקום לחזור על אותו ספקטרום, סעיף זה מטפל בארבע דאגות נוספות המתעוררות במימוש סוכן קוד: כיצד לנהל יציאת רשת, כמה ממערכת הקבצים למפות, כיצד להגביל משאבים, וכיצד ליישב סשנים מתמידים עם בידוד.
...
מעקב אחר חוט זה — מבפנים החוצה ובסופו של דבר בחזרה לסוכן עצמו — מקל על ראיית הערך המאוחד של קוד כמטא‑יכולת. על בסיס זה, פרק 9 בוחן אילו ראיות תפעוליות צריכות להפעיל שינוי עצמי וכיצד שינויים מועמדים נכנסים לגרסה חדשה דרך בדיקות, שחרור ורולבק.
...
>
> **גישה טכנית**: השתמשו במימוש מהפרויקט `PermissionEmbeddedDataObjects` כדי לספק שכבת middleware של מאגר אובייקטים ב‑Python מעל PostgreSQL. טיפוסי נתונים מצהירים על כללי ההרשאה שלהם, על הקשר הגישה, על מאמתים, על יחסי אובייקטים ועל תגובות. כל קריאה או כתיבה עוברת דרך בדיקות הרשאה ואימות, טיפול בהתמדה ובשלמות התייחסותית, ותגובות אסינכרוניות מבוקרות. הריצו תחילה את הדגמת צינור הגיוס הדטרמיניסטית ללא LLM; אופציונלית, בקשו ממודלים לייצר פעולות יריביות מול SQL חשוף ומול ה‑API של PEDO, ואז השוו את מצבי מסד הנתונים המתקבלים. ההשוואה המרכזית אינה האם מטפל שנוצר מכיל את משפט ה‑`if` הנכון, אלא האם אותה בקשה מתקבלת או נדחית באופן אמין כשהיא מגיעה לשכבת הנתונים היציבה.
>
> **קריטריוני קבלה**: עדכון תקף של צינור הגיוס מצליח; דילוג על מעבר מצב של מועמד, כתיבת שכר מחוץ לטווח התפקיד, וקריאה בין דיירים — כולם נדחים על ידי שכבת הנתונים. בדיקות הליבה של הרשאות, אימות, בידוד דיירים, שלמות התייחסותית ותגובות עוברות. המימוש כלול בפרויקט הנלווה של פרק 5 תחת `permission-embedded-data-objects`.
