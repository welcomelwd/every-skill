...
זיהוי קונפליקטים משתמש בגישת גרסאות — גרסאות היסטוריות נשמרות בעוד שהגרסה העדכנית מסומנת. עבור מידע מסוים (למשל, כתובת נוכחית), נשמרת רק הגרסה העדכנית; עבור מידע אחר (למשל, היסטוריה תעסוקתית), ההיסטוריה המלאה נשמרת.
...
טכנולוגיית הליבה לבניית בסיס ידע משותף היא אחזור מוגבר ביצירה (Retrieval‑Augmented Generation,‏ RAG). הרעיון המרכזי הוא לשלב את יכולות החשיבה והיצירה של מודלי שפה גדולים עם הרוחב והעדכניות של בסיס ידע חיצוני — לנתוני האימון של המודל יש תאריך חיתוך, בעוד שבסיס הידע ניתן לעדכון בכל עת.
...
הדפוס זהה בשתי הדוגמאות: **אחזור קטעים רלוונטיים ← הזרקה להקשר ← ה‑LLM מייצר תשובה על בסיס ההקשר**. ערכו המרכזי של RAG הוא באפשרות לאפשר ל‑LLM להשתמש בידע שלא ראה במהלך האימון (תוכן ויקיפדיה העדכני ביותר, מסמכים פנימיים של חברה) מבלי צורך לאמן מחדש את המודל.

איכות המאחזר קובעת ישירות את האפקטיביות של RAG — אם אינו יכול לאחזר קטעים רלוונטיים, גם ל‑LLM החזק ביותר אין עם מה לעבוד. סעיף זה מתחיל בצעד הראשון של הכנסת מסמכים לבסיס הידע — פירוק לחלקים (chunking) — ולאחר מכן פונה לשתי גישות האחזור העיקריות, שיכונים צפופים (הבנה סמנטית) ושיכונים דלילים (התאמת מילות מפתח), וכיצד לשלב ביניהן.
...
BM25‏ (Okapi BM25) ניתן לראייה כתיקון קלאסי לשתי מגבלות אלה. הוא משמר שקלול IDF למונחים נדירים תוך הוספת רוויית תדירות מונחים ונרמול אורך מסמך:
...
האינטואיציה אינה משתנה — ככל שהמונח נדיר יותר, כך משקלו גבוה יותר — רק אופן מדידתו. המונה הופך למספר המסמכים ש*אינם* מכילים את המונח, $N - \text{DF}(t)$, ולא לגודל הקורפוס $N$, כך שהיחס מציין פי כמה יותר מסמכים חסרים את המונח מאשר מכילים אותו; הוספת 0.5 למונה ולמכנה כאחד מחליקה את התוצאה, ושומרת על הנוסחה מוגדרת בשני הקצוות $\text{DF}(t) = 0$ ו‑$\text{DF}(t) = N$. המחיר הוא שמונח המופיע ביותר ממחצית המסמכים ($\text{DF}(t) > N/2$) מקבל משקל שלילי, ולכן מימושים בדרך כלל חוסמים אותו ברצפה. וריאנט זה מגיע ממודל האחזור ההסתברותי וידוע בספרות כמשקל Robertson–Spärck Jones.
...
>
> במהלך שאילתה, היומן מפרט כל שלב בחישוב ה‑BM25. תוך שימוש בשאילתה "model distillation" שוב כדוגמה — היומן הבא מגיע מקורפוס דגימה קטן (N=10 מסמכים) הכלול בפרויקט, ולכן מספר הפגיעות קטן בהרבה מתרחיש 100 המאמרים שהוזכר קודם. כדי להקל על חישוב ידני חוזר, הדוגמה מקבעת פרמטרי BM25‏ k1=1.5,‏ b=0.75, ואורך מסמך ממוצע avgdl=250 מילים; IDF משתמש בצורת ה‑BM25 שניתנה לעיל, IDF=ln((N−df+0.5)/(df+0.5)), כאשר df הוא מספר המסמכים המכילים את המילה:
>
> ```
...
**אחזור דליל נלמד.** פרק זה משתמש ב‑BM25 הקלאסי כנציג האחזור הדליל משום שהוא אינו דורש אימון, הוא שקוף ובר‑שחזור, והוא המתאים ביותר להסבר עקרונות האחזור הדליל. עם זאת, האחזור הדליל עצמו נכנס לשלב "נלמד": מודלים כגון SPLADE, יחד עם ענף הפלט הדליל של BGE‑M3, משתמשים ברשתות נוירונים כדי להקצות משקלים לכל מונח — לא עוד ניקוד המבוסס רק על תדירות מונחים ותדירות מסמכים כמו BM25, אלא מתן אפשרות למודל לשפוט "עד כמה מילה זו חשובה בטקסט הזה", ואף להקצות משקלים שאינם אפס למונחים הקשורים סמנטית אך אינם מופיעים בטקסט המקורי (הרחבת מונחים). התוצאה עדיין וקטור דליל שרוב ממדיו אפס, המשמר יכולת פרשנות מילונית והתאמה מדויקת תוך זכייה בהכללה סמנטית מסוימת מהרשת הנוירונית. חשבו על כך כעל נקודת מפגש בין המסלול הדליל למסלול הצפוף.
...
השני הוא **מיזוג תוצאות**, המשלב את שני מערכי התוצאות למאגר מועמדים אחיד. הקושי הוא שהציונים משני המסלולים אינם ברי השוואה ישירה: ציוני הדמיון מאחזור צפוף (למשל, דמיון קוסינוס, הנע תיאורטית בין ‎−1 ל‑1, אך שיכוני טקסט מנורמלים בפועל נופלים בדרך כלל בין 0 ל‑1) וציוני ה‑BM25 מאחזור דליל (שיכולים להיות כל ערך מ‑0 עד עשרות) בעלי סקאלות והתפלגויות שונות לחלוטין. שתי שיטות מיזוג נפוצות הן: ראשית, נרמול הציונים מכל מסלול בנפרד ולאחר מכן ביצוע סכום משוקלל; שנית, מיזוג דירוג הופכי (Reciprocal Rank Fusion,‏ RRF) — השלכה מוחלטת של הציונים המקוריים והתבוננות בדירוגים בלבד. הציון המשולב לכל מסמך הוא סכום ההופכיים המוחלקים של דירוגיו בכל מערך תוצאות, כלומר score = Σ 1/(k + rank), כאשר k הוא קבוע החלקה (לרוב 60), המשמש לצמצום פער הציונים בין המקומות המובילים. RRF פשוט ועמיד, אך הוא משתמש רק במידע דירוג, ומשליך את אות הרלוונטיות העשיר שבציונים המקוריים.
...
אנלוגיה: מגייס המרפרף על קורות חיים לסינון ראשוני הוא מקודד הדו; מראיין המנהל שיחה עמוקה עם כל מועמד הוא המקודד הצולב. הראשון מסנן בקנה מידה גדול על בסיס מאפיינים שחולצו מראש; האחרון מאפשר לשאילתה ולכל מסמך מועמד להיפגש "פנים אל פנים" ולהיות מוערכים מילה אחר מילה. המדרג מחדש מעסיק את ארכיטקטורת ה"מקודד הצולב", בניגוד חד ל"מקודד הדו" המשמש בשלב האחזור. **מקודד דו** מייצר וקטורים עצמאיים לשאילתה ולמסמך ומחשב דמיון באמצעות פעולות וקטוריות — מהיר מאוד, אך אינו יכול ללכוד יחסי התאמה עמוקים, ומתאים לסינון ראשוני מנתונים עצומים. **מקודד צולב** **משרשר את השאילתה ואת המסמך המועמד לקטע טקסט יחיד** ומזין אותו למודל, ומאפשר למודל להשוות מילה אחר מילה ולהוציא ציון רלוונטיות מקיף[^ch3-cross-encoder] — איטי בהרבה, אך מדויק יותר בשיפוטי רלוונטיות. מודלי דירוג מחדש נפוצים כגון [BAAI/bge-reranker-v2-m3](https://huggingface.co/BAAI/bge-reranker-v2-m3) מאמצים ארכיטקטורה זו.

מנגנון "קשב משותף" זה מאפשר למקודד הצולב ללכוד קשרים סמנטיים עדינים שמקודד הדו אינו יכול לתפוס, וכתוצאה מכך דירוג סופי מדויק בהרבה מכל שיטת אחזור בודדת.

[^ch3-cross-encoder]: במימושים של מודלים מסוג BERT, הקלט המשורשר מופרד על ידי טוקנים מיוחדים (למשל, `[CLS] query text [SEP] document text [SEP]`, כאשר `[CLS]` מסמן את תחילת הרצף ו‑`[SEP]` מסמן את הגבול). זהו פרט מימוש בסיסי ואינו הכרחי להבנת תהליך האחזור.
...
דוחות תעשייה מזכירים בדרך כלל גם "שיעור כשל אחזור". לדוגמה, בנתוני Anthropic המצוטטים בהמשך פרק זה, שיעור כשל האחזור מתייחס לשיעור השאילתות שבהן המידע הנכון אינו מופיע ב‑20 תוצאות האחזור המובילות — במהותו 1 − recall@20. כשאתם נתקלים במספרים כאלה, קבעו במדויק לאיזה מדד הם ממופים ומהו k לפני השוואה בין מקורות.
...
עד כה כל מה שאחזרנו היה טקסט פשוט. ידע מהעולם האמיתי שוכן בצורות רבות בהרבה.
...
**מקרה 1: בעיית ספירת החתולים השחורים והלבנים.** בפרק 2, השתמשנו בדוגמת ספירת החתולים השחורים והלבנים כדי להמחיש ש"קשב הוא מנגנון אחזור רך, ומידע סטטיסטי צריך להיות מחולץ מראש" — גם אם כל 100 המקרים נטענים לחלון ההקשר, המודל מתקשה לבצע ספירה מדויקת. אותה בעיה מופיעה מחדש בקנה המידה של בסיס הידע, ומתווספים לה כמה מכשולים חדשים. נניח שבבסיס הידע יש 100 מסמכי מקרה עצמאיים (90 חתולים שחורים, 10 לבנים, כל אחד חלק טקסט עצמאי), והמשתמש שואל "מהו היחס בין חתולים שחורים ללבנים?" ראשית, **קטיעת top‑k** — עם ערך top‑k קטן, כגון 20, רוב המקרים לא יאוחזרו כלל. שנית, **ציוני אחזור בלתי אחידים** — גם עם k גדול יותר, מקרים בודדים מתוארים באופן שונה, ציוניהם משתנים מאוד, וחלקם עדיין מוחמצים. באופן היסודי ביותר, קיים **אי‑התאמה בצבירה חוצת‑מסמכים** — שאלות סטטיסטיות דורשות "ספירה על פני כל המסמכים", בעוד שטבעו של האחזור הוא "מציאת המעטים הרלוונטיים ביותר", ובכך נוצרת סתירה מובנית. המודל יכול להסיק רק מסקנות שגויות על בסיס מדגם חלקי (למשל, בראותו רק 15 חתולים שחורים ו‑3 לבנים). אם מאונדקס סיכום שנוצר מראש כגון "סך הכול 100 חתולים: 90 שחורים (90%) ו‑10 לבנים (10%)", אחזור יחיד מניב מידע מדויק.

**מקרה 2: בעיית הגבול בזכאות להנחת Xfinity.** הפעם בסיס הידע הוא ארכיון פניות תמיכה: כמה מאות פניות, שכל אחת מתעדת תוצאה אמיתית אחת — הוותיק ג'ון אושר, הרופאה שרה קיבלה את ההנחה, המורה מייק נאמר לו שאינו זכאי, וכן הלאה. כל פנייה מציינת את המסקנה של מקרה פרטני אחד; אף אחת מהן אינה מציינת את היקף הזכאות עצמו. כאשר אחות שואלת "האם אני זכאית?", מכשולים מרובים מצטברים. ראשית, **הטיית השכן הקרוב** — "אחות" קרובה סמנטית ביותר ל"רופאה", ולכן הפנייה של שרה מדורגת ראשונה והמודל מסיק כדין שגם אחיות זכאיות; אילו הפנייה של מייק הייתה מדורגת גבוה יותר במקרה, אותה שאלה הייתה מקבלת את התשובה ההפוכה. **התשובה נקבעת לפי איזו פנייה יושבת הכי קרוב לשאילתה, ולא לפי המדיניות עצמה.** שנית, **סמנטיקת גבול חסרה** — מכשול ש‑k גדול יותר אינו יכול לתקן: אמירה מהצורה "רק ..., כל שאר המקצועות אינם זכאים" נושאת כמת אוניברסלי ושלילה, והיא אינה שוכנת באף פנייה בודדת, אלא רק בסגור של הקורפוס כולו. הארכיון מלכתחילה אינו עונה על השאלה "האם אחות נחשבת", ולכן כפיית המודל להסיק כלל אוניברסלי מקומץ מקרים פרטניים מניבה מסקנה שמעולם לא הייתה תקפה. שלישית, **אותות שלמות חסרים** — למודל אין דרך לדעת האם ראה את הכלל בשלמותו, ולכן הוא לעולם אינו שואל; הוא פשוט עונה בביטחון מתוך הפניות המעטות שבידיו. התיקון שייך שוב לזמן האינדוקס: קראו את ארכיון הפניות כולו באופן לא מקוון, ותוך התייחסות למדיניות הזכאות הרשמית כסמכות (ולא אקסטרפולציה מקומץ המקרים שאוחזרו — שהיא בדיוק זיהום הידע שמפניו מזהירים בהמשך), זקקו כרטיס כלל יחיד: "הנחות Xfinity חלות על משרתים בשירות פעיל ועל ותיקים, ועל אנשי מקצוע רפואיים מורשים לרבות אחיות; מקצועות אחרים כגון מורים אינם זכאים; מקצועות שאינם מפורטים דורשים בדיקה אנושית." ברגע שהגבול והנפילה החלופית נכתבים שניהם, אחזור יחיד מניב את הכלל המלא ולא משנה על איזה מקצוע שואלים — המודל אינו נאלץ עוד להסיק, אלא רק להתאים.
