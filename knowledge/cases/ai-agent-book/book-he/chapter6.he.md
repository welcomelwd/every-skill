...
קורפוס האימון של מודל הוא כמעט כולו מבוסס תורים — שאלה ואחריה תשובה, קריאה לכלי ואחריה תוצאת כלי, דובר אחד המסיים לפני שהאחר מתחיל. לפיכך המדיניות שהמודל לומד מניחה שהעולם ימתין לו. הסביבה האמיתית אינה ממתינה: דואר מגיע בזמן שהוא חושב, המשתמש נכנס לדבריו באמצע משפט, הדף כבר השתנה בין שני צילומי מסך, הכוס נהפכת בזמן שהזרוע מושטת אליה. **ארבעת הסעיפים של פרק זה הם ההנחה הזו מתרופפת, סקאלת זמן אחת בכל פעם.**
...
| Omni מקצה לקצה | מודל אחד מאזין, חושב ומדבר | השהיה נמוכה יותר ושימור טוב יותר של גוון, רגש וצליל סביבתי | עדיין מבוססת תורים; האימון והניפוי יקרים יותר |
| דו‑כיוונית מלאה | מאזינה, מדברת ומחליטה ברציפות | דיבור חופף, קטיעה טבעית וזרמים רציפים | האימון, הבקרה וההערכה מורכבים יותר |
...
כאשר מערכת מדורגת עוברת מביצוע טורי להזרמה, השינוי החשוב ביותר אינו הפיכת כל פונקציה ל‑`async`, אלא מתן אפשרות ש**תוצאות מצטברות יאבדו את תוקפן ויבוטלו**.
...
איור 6‑7 מתאר את המקרה הטורי לחלוטין שבו כל שלב ממתין לקודמו. מערכת ייצור יכולה לשמר את הפיצול המודולרי תוך הפקת תוספות מוקדם ככל האפשר:

- **ASR מזרים** מפיק ברציפות תמליל זמני בזמן שהמשתמש מדבר, ואז מאשר את הטקסט הסופי בגבול התור.
- **פלט LLM מקוטע** שולח את המשפט הראשון הניתן לאמירה ל‑TTS בלי להמתין לתשובה המלאה.
- **TTS מצטבר** מחזיר מקטעי אודיו כך שהייצור, הסינתזה וההשמעה המאוחרים יותר חופפים.

"הזרמה בכל שלב" אינה הופכת את ה‑ASR, ה‑LLM וה‑TTS למקביליים לחלוטין מתחילה ועד סוף. בצינור מדורג תקני, ה‑ASR חופף לדיבור המשתמש וה‑TTS חופף לטוקנים המאוחרים של ה‑LLM, אך התשובה הסופית עדיין תלויה בתמליל יציב. מערכת אגרסיבית יותר מפעילה את ה‑LLM מתמליל חלקי; אם הטקסט משתנה מאוחר יותר, עליה לבטל, להתחיל מחדש, או לתקן את הייצור. ספקולציה דורשת מנגנוני commit, ביטול תוקף וגלגול לאחור מפורשים; הפעלת \`stream\` לבדה אינה מספקת אותם.

הזרמה רגילה גם אינה יכולה להסיר את המתנת השתיקה של ה‑VAD. לחזית מסורתית של VAD + ASR יש שלוש בעיות:

1. **השהיה מצטברת:** עליה להמתין דרך השתיקה לפני שתאשר את הסיום.
...
מודל מזרים באמת זקוק למקודד סיבתי או מקוטע עם פענוח מצטבר. המפענח של Whisper הוא אוטורגרסיבי, אך המקודד שלו מצפה למקטע אודיו שלם, ולכן אין לכנותו מודל הזרמה סיבתי. ‏ASR מסוג RNN‑T ו‑Conformer מזרים משמשים בתעשייה זה זמן רב; המיקוד כאן הוא בהאזנה סמנטית הבנויה על עמוד שדרה של LLM.

מודל אודיו מזרים מבוסס LLM יכול להנפיק טקסט ואירועים סמנטיים מאודיו רציף, ובכך למקם את הזיהוי וחלק מההבנה במודל אחד. הוא שומר על הקשר השיחה מההתחלה ויכול להשתמש בידע עולם עבור מותגים, שמות ושמות עצם פרטיים. קיטוע מדומה עדיין אינו הבטחת ביצועים למודל סיבתי.

אם המטרה היחידה היא להחליט האם המשתמש סיים, ניתן לבנות זיהוי נקודת קצה לתוך המזהה המזרים. המודל משלב סמנטיקה ושתיקה כדי לשפוט האם אמירה שלמה. תוויות האימון חייבות להכיל רק מידע הגלוי בזמן ההחלטה, אחרת חוכמה שבדיעבד תפיק שיפוט שאינו ניתן לשחזור בזמן אמת[^ch6-11]. זה קל יותר מ‑LLM מלא בעל יכולת אודיו.
...
[^ch6-11]: לאבחון של שיבוץ שיפוט התור בתוך המזהה ולבעיית התוויות המבוססות על חוכמה שבדיעבד, ראו Bojie Li and Noah Shi. *The Trade-off Was in the Labels: Causal Supervision for Turn-Aware Streaming ASR.* 2026 (בהכנה).
...
גם עם תפיסה מזרימה, צינור מדורג מעביר את ההאזנה, החשיבה והדיבור דרך ממשקים בדידים; רגש, אינטונציה וצליל סביבתי עלולים לאבוד כשאודיו הופך לטקסט פשוט. ‏Omni משתמש במודל אחד כדי להאזין לאודיו, לייצר תשובה ולומר אותה, מה שיכול לשמר אותות אלה במחיר של עלויות אימון, ניפוי והחלפת רכיבים גבוהות יותר (איור 6‑9).

היתרון של מקצה לקצה הוא בעיקר השהיה ומידע שאינו טקסטואלי, לא בהכרח דיוק. דירוג עצמי (self‑cascade) מתמלל תחילה באמצעות אותו מודל ואז עונה מהתמליל: כאשר הטקסט נושא את מידע המשימה, הוא עשוי לתקן שגיאת תפיסה; כאשר התשובה תלויה בקצב דיבור, ברגש או בצליל סביבתי, צוואר הבקבוק הטקסטואלי מאבד ראיות באופן בלתי הפיך. השאלה המרכזית אינה האם קיים ייצוג ביניים, אלא איזה מידע הוא נושא[^ch6-13].

‏Omni עדיין מניח חלוקה לתורים ומשתמש בדרך כלל ב‑VAD או בזיהוי נקודת קצה סמנטי כדי להקצות את רשות הדיבור. הפסקה ברצף מספרים מדובר עדיין עלולה להתפרש בטעות כסיום; תפיסה מזרימה משפרת את השיפוט אך אינה מסירה את התורים.

[^ch6-13]: למדידה חוצת‑מודאליות מלאה של מתי והיכן יתרונות הדיוק של דירוג ושל מקצה לקצה מתהפכים, וכיצד אופי המשימה מנבא את הכיוון, ראו Li, Bojie and Noah Shi. *The Cascade Gap: When and Why Self-Cascades Help Multimodal Agents.* 2026 (בהכנה).

![איור 6‑9: השוואת מודלי דיבור אומני‑מודאליים מקצה לקצה](images/fig6-9.svg)

ממשקי API לדיבור בזמן אמת ממוקמים בין מערכות מדורגות ל‑Omni: המודל מטפל באודיו באופן טבעי, אך בקרת האינטראקציה עדיין נשענת על VAD, קטיעה וקריאות אסינכרוניות לכלים. ה‑Thinker‑Talker של Qwen3-Omni והנתיב המקומי של MiniCPM-o מראים שגישה זו יכולה לשלב חשיבה, הבעה וקלט רב‑מודאלי בגדלי מודל שונים. ההשוואה המועילה אינה טבלת דירוג; היא כיצד נתיבי מקצה לקצה ודירוג עצמי נכשלים במשימות שונות.
...
‏Step-Audio 2 מדגים נתיב מקצה לקצה המעבד אודיו גולמי ומנפיק טקסט ודיבור; הוא מתמקד ברגש, בקצב דיבור, באינטונציה ובצליל סביבתי מעבר לסמנטיקה. ‏Step-Audio R1 מרחיב נתיב זה על ידי הפנמת ההיסק במודל האודיו; הוא ישמש כדוגמה ל"חשיבה תוך כדי דיבור".
...
איכות האינטראקציה ותקרת האינטליגנציה הם ממדים שונים. מודל החזית חייב להגיב בעוד המשתמש עדיין מעורב; מודל הרקע יכול להקדיש זמן רב יותר לחשיבה. שלושת העיצובים שלהלן הם פשרות, לא התקדמות לינארית. השניים הראשונים יכולים לעטוף מודל מדורג או Omni; רק השלישי מאחד חשיבה והבעה במודל אודיו אחד מקצה לקצה.

| עיצוב | חזית | רקע | הסיכון העיקרי |
| --- | --- | --- | --- |
| ממלא זמן מהיר, תיקון איטי | לתת תשובה מיידית | לחשוב מחדש ולהשלים אותה | סתירה |
| אינטראקציה מהירה, עצה איטית | לשמור על השיחה חיה ולבחור ניסוח | לספק עצה או תוצאות כלים | ממשק מוגבל |
| איחוד חשיבה והבעה | לחשוב ולדבר יחד | לחלוק מצב מודל עם ההבעה | עלות אימון והחלפה גבוהה |
...
#### פתרון 3: איחוד מקצה לקצה של חשיבה והבעה (באמצעות Step-Audio R1)
...
באופן אידיאלי, המודל מסיק רגש מגובה הצליל, מהקצב ומהאינטונציה ולא רק מהתמליל. "חשיבה בפרוקסי טקסטואלי" מחליפה ניתוח של מלודיה ואקוסטיקה במילים שליליות שבמילות השיר. ‏MGRD בוחר עקבות היסק המצטטים בפועל מאפיינים אקוסטיים, מאמן עליהם, ומשתמש בלמידת חיזוק כדי למנוע ניחוש ללא חשיבה.

‏MPS מאפשר למוח המתכנן להנפיק ברציפות מקטעי מחשבה; מוח ההבעה משלב כל מקטע עם התשובה החלקית ומייצר מיד דיבור. הצינור רץ במקביל, כך שהמאזין אינו צריך להמתין לשרשרת ההיסק כולה לפני ששומע את המשפט הראשון.

מודל מאוחד מממש "חשיבה תוך כדי דיבור" באופן הישיר ביותר, אך יש לאמן מחדש יחד את החשיבה ואת ההבעה בזמן אמת. עיצוב מנותק מקל על החלפת מוח הרקע; עיצוב מאוחד מתאים לתרחישים מתמחים הדורשים את האינטראקציה הטבעית ביותר. אלה פשרות, לא תחליפים פשוטים.

### סינתזת דיבור דמוית‑אדם יותר

‏TTS מסורתי עלול לחשוף את זהותו המכונתית בהיותו חלק מדי ובעצירתו מעט מדי. הפסקות, מילות מילוי וחזרה מזדמנת מאותתות על אי‑ודאות ועל חשיבה בדיבור אנושי.

ה‑LLM הראשי יכול להנפיק סמני בקרה בנוסף לטקסט, כגון **THINKING**,‏ **EMO:happy** ו‑**SPEED:0.8x**; ‏TTS ממפה אותם להפסקות, לפרוזודיה, לקצב דיבור, לצחוק, לאנחות ולאודיו לא‑מילולי אחר. המימוש יכול להיות TTS שאומן להבין סמני בקרה, או שיבוט קול עם קטעי ייחוס לרגשות ולסגנונות שונים.

> **ניסוי 6‑6 ★★: TTS מונחה טוקני בקרה עם Fish Audio**
>
> השתמשו ב‑Fish Audio S1 כדי לבנות ספריית קול מרובת ייחוסים והשוו שלוש תצורות: ללא סמני בקרה, קטע ייחוס אחד, וקטעי ייחוס מרובים. שכבת הביצוע בוחרת מהסמנים רגש, קצב דיבור וסגנון תואמים.
>
> תצורת הייחוסים המרובים קיבלה את הציון הגבוה ביותר בשלושה מעברי האזנה עיוורת מאוזני‑מיקום (דמיון לשירות לקוחות אנושי 4.67/5), אך הסדר המלא שתוכנן לא שוחזר משום שהזרוע ללא סמנים קיבלה ציון גבוה יותר מזרוע הייחוס היחיד. תוצאה זו מרמזת שבקרה הבעתית מסייעת, אך מחקר האזנה קטן אינו מסקנה כללית על איכות דיבור. ספריית 24 הייחוסים המלאה, מדיית A/B/C ורשומת הקבלה נמצאות ב‑[chapter6/controllable-tts](../chapter6/controllable-tts/).
...
מה שדורש עיצוב מחדש אינו ממשק הפעולה אלא **ממשק התצפית**[^ch6-9]. ממשק תצפית סוכן‑מחשב (AOI) ממיר תצפית סביבתית רציפה לאירועים בדידים שהמודל יכול לטפל בהם. הטכניקות המרכזיות שלו הן: **לכידת פריימי מפתח בין‑פריימית**, המדלגת על מסכים שכמעט לא השתנו ומשתמשת במודל קטן כדי לשמר רק שינויים משמעותיים; **תמלול דיבור מגודר‑עוצמה**, המפעיל זיהוי רק כשיש צליל; ו**תיאור פריימים כטקסט**, כך שהתיאור נותר בזיכרון לאחר שהתמונה המקורית עוזבת את ההקשר, ובכך דוחס את היסטוריית האינטראקציה הרב‑מודאלית.
