# Validate USD usd-validation-nvidia

## Purpose

Run the selected NVIDIA Omniverse usd-validation-nvidia checks from the
...
5. Store raw outputs on disk and write a compact summary before reading results
   into context.
6. Feed summarized findings to `usd-optimize-interpret-validators` and the optimization
   report.
...
`../README.md` → `usd-validation-nvidia Load Rules`.
...
- This reference reports usd-validation-nvidia findings only; it does not apply
  `--fix` or repair USD content unless the user explicitly asks for auto-repair.
- Usd Optimize performance validators run through `usd-optimize-run-validators` when
...
Pass compact findings to `usd-optimize-interpret-validators`. Revalidate same-or-narrower
