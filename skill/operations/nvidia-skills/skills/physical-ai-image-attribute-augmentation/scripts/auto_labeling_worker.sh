...
#
# Image Attribute Augmentation auto-labeling worker: generate person-attribute captions and synonymous
...
cd /workspace
# The runtime (pre-built venv at /opt/venv, on PATH) is provided by the image at
# build time — see UV_PROJECT_ENVIRONMENT/UV_NO_SYNC above. We deliberately do
# not execute the in-image /workspace/docker/entrypoint.sh here: the OSMO task
# runs this worker as the command, the environment is already prepared, and
# augmentation_worker.sh runs the same way without invoking any entrypoint.
...
# Fail closed: if no captions were produced, exit non-zero so OSMO marks the task
# failed instead of the run appearing successful with missing captions.
if [ "${TOTAL}" -eq 0 ] || [ "${PASSED}" -eq 0 ]; then
    echo "ERROR: No captions generated (total=${TOTAL} passed=${PASSED} failed=${FAILED})" >&2
    exit 1
fi
