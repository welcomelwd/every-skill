...
#
# Set credentials without exposing secret values in the process argv. `osmo
# credential set --payload k=v` puts values on the command line (visible via
# `ps`); instead write each value to a private temp file and use --payload-file
# (key=path), the CLI's documented file-based payload input.
osmo_credential_set_secure() {
  local name="$1" ctype="$2"; shift 2
  local d rc=0 i=0 kv key val f
  d="$(mktemp -d)"; chmod 700 "$d"
  local -a pf=()
  for kv in "$@"; do
    key="${kv%%=*}"; val="${kv#*=}"
    f="${d}/${i}"; printf '%s' "$val" > "$f"; chmod 600 "$f"
    pf+=("${key}=${f}"); i=$((i+1))
  done
  osmo credential set "$name" --type "$ctype" --payload-file "${pf[@]}" || rc=$?
  rm -rf "$d"
  return $rc
}
...
if ! grep -qx 'nvcr_io' <<<"$present"; then
  if [[ -n "${NGC_API_KEY:-}" ]]; then
    ...
    osmo_credential_set_secure nvcr_io REGISTRY \
      'registry=nvcr.io' 'username=$oauthtoken' "auth=${NGC_API_KEY}"
  else
...
if ! grep -qx 'hf_token' <<<"$present"; then
  ...
  osmo_credential_set_secure hf_token GENERIC \
