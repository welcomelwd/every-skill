...
# Load setup vars safely: parse KEY=VALUE lines, never `source`/execute the file
# (defense against arbitrary-code execution from a tampered setup file).
SETUP_VARS="${SETUP_DIR}/setup.vars"
if [ -f "${SETUP_VARS}" ]; then
    while IFS='=' read -r _k _v; do
        case "${_k}" in ''|\#*) continue ;; esac
        if [[ "${_k}" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]]; then export "${_k}=${_v}"; fi
    done < "${SETUP_VARS}"
fi
...
if [ -f "${DIST_CONFIG}" ]; then
    # Pass paths/values via the environment (os.environ), never interpolated into
    # the Python source string — a tampered path can't inject Python code.
    DIST_CONFIG="${DIST_CONFIG}" PANES_DIR="${PANES_DIR}" \
    AUG_CONFIGS_DIR="${AUG_CONFIGS_DIR}" AUG_OUTPUTS_DIR="${AUG_OUTPUTS_DIR}" \
    N_AUGMENTATIONS="${N_AUGMENTATIONS}" \
    uv run python -c "
import yaml, os, random, itertools

with open(os.environ['DIST_CONFIG']) as f:
    dist = yaml.safe_load(f)

panes_dir = os.environ['PANES_DIR']
configs_dir = os.environ['AUG_CONFIGS_DIR']
outputs_dir = os.environ['AUG_OUTPUTS_DIR']
n_aug = int(os.environ['N_AUGMENTATIONS'])
...
for cfg_file in "${AUG_CONFIGS_DIR}"/*.yaml; do
    ...
    # Pass cfg_file as argv[1] (never interpolated into the Python source string).
    PERSON_ID=$(${PY} -c "import sys, yaml; print(yaml.safe_load(open(sys.argv[1]))['person_id'])" "${cfg_file}")
    AUG_IDX=$(${PY} -c "import sys, yaml; print(yaml.safe_load(open(sys.argv[1]))['aug_index'])" "${cfg_file}")
    INPUT_IMG=$(${PY} -c "import sys, yaml; print(yaml.safe_load(open(sys.argv[1]))['input_image'])" "${cfg_file}")
    OUT_SUBDIR=$(${PY} -c "import sys, yaml; print(yaml.safe_load(open(sys.argv[1]))['output_dir'])" "${cfg_file}")

    echo "--- Augmenting ${PERSON_ID} aug_${AUG_IDX} ---"

    # Build per-variable CLI args as a NUL-delimited array so values are passed
    # verbatim to uv run. No `eval`: nothing here is re-parsed by the shell, so
    # cookbook/config values can never be shell-evaluated (defense in depth
    # behind Gate 6's input validation).
    VARIABLE_ARGS=()
...
for k, val in (cfg.get('variables', {}) or {}).items():
    sys.stdout.write(f'captioning.llm.variables.{k}=[{val}]\0')
" "${cfg_file}")

    AUG_EXIT=0
    uv run modules/cli.py --config "${AUG_CONFIG}" \
...
# Fail closed: if nothing was produced, exit non-zero so OSMO marks the task
# failed instead of the run appearing successful with no usable outputs.
if [ "${TOTAL}" -eq 0 ] || [ "${PASSED}" -eq 0 ]; then
    echo "ERROR: No usable augmentation outputs produced (total=${TOTAL} passed=${PASSED} failed=${FAILED})" >&2
    exit 1
fi
