...
"""
ms-agent × nanobot  —  MCP integration test
...
Usage:
  ...
  python test_mcp_tools.py --list        # list tools only
...
async def connect(stack):
    """Start an MCP Server subprocess the same way nanobot would."""
...
async def list_tools(session):
    ...
    print(f'  (nanobot will prefix these as mcp_ms-agent_<name>)')
    print(f'{"=" * 64}\n')
    for tool in result.tools:
        desc = (tool.description or '')[:55]
        nanobot_name = f'mcp_ms-agent_{tool.name}'
        print(f'  {nanobot_name:48s} {desc}')
    print()
    return result.tools
...
async def main():
    parser = argparse.ArgumentParser(
        description='ms-agent x nanobot MCP integration test',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='Examples:\n'
               '  python test_mcp_tools.py --list        # list tools\n'
               '  python test_mcp_tools.py --test ws     # web search\n'
               '  python test_mcp_tools.py --test fs,ws  # combined\n'
