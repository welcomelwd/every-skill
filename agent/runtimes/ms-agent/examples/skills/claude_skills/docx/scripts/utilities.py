...
class XMLEditor:
    ...
    def get_node(
        ...
        if not matches:
            ...
            filters = []
            if line_number is not None:
                line_str = (f'lines {line_number.start}-{line_number.stop - 1}'
                            if isinstance(line_number, range) else
                            f'line {line_number}')
    ...
    def _parse_fragment(self, xml_content):
        ...
        nodes = [
            self.dom.importNode(child, deep=True) for child in
            fragment_doc.documentElement.childNodes  # type: ignore
        ]
