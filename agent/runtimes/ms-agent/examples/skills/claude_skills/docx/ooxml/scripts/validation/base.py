...
class BaseSchemaValidator:
    ...
    UNIQUE_ID_REQUIREMENTS = {
        ...
        'sldmasterid':
        ('id', 'global'),  # Slide master IDs must be globally unique
        'sldlayoutid':
        ('id', 'global'),  # Slide layout IDs must be globally unique
    ...
    SCHEMA_MAPPINGS = {
        ...
        'custom.xml':
        'ISO-IEC29500-4_2016/shared-documentPropertiesCustom.xsd',
    ...
    def validate_xml(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                errors.append(f'  {xml_file.relative_to(self.unpacked_dir)}: '
    ...
    def validate_namespaces(self):
        ...
        for xml_file in self.xml_files:
            try:
                ...
                declared = set(
                    root.nsmap.keys()) - {None}  # Exclude default namespace

                for attr_val in [
                        v for k, v in root.attrib.items()
        ...
        for rels_file in rels_files:
            try:
                ...
                for rel in rels_root.findall(
                        './/ns:Relationship',
                        namespaces={
                            'ns': self.PACKAGE_RELATIONSHIPS_NAMESPACE
                        },
                ):
                    target = rel.get('Target')
                    if target and not target.startswith(
                        ('http', 'mailto:')):  # Skip external URLs
    ...
    def validate_content_types(self):
        ...
        try:
            ...
            for xml_file in self.xml_files:
                path_str = str(xml_file.relative_to(
                    self.unpacked_dir)).replace('\\', '/')

                # Skip non-content files
                if any(skip in path_str for skip in
                       ['.rels', '[Content_Types]', 'docProps/', '_rels/']):
                    continue

                try:
                    root_tag = lxml.etree.parse(str(xml_file)).getroot().tag
                    root_name = root_tag.split(
                        '}')[-1] if '}' in root_tag else root_tag
        ...
        if new_errors:
            if verbose:
                ...
                for error in list(new_errors)[:3]:
                    truncated = error[:250] + '...' if len(
                        error) > 250 else error
                    print(f'  - {truncated}')
    ...
    def validate_against_xsd(self):
        ...
        for xml_file in self.xml_files:
            ...
            for error in list(new_file_errors)[:3]:  # Show first 3 errors
                new_errors.append(f'    - {error[:250]}...'
                                  if len(error) > 250 else f'    - {error}')
    ...
    def _get_schema_path(self, xml_file):
        ...
        if xml_file.parent.name in self.MAIN_CONTENT_FOLDERS:
            return self.schemas_dir / self.SCHEMA_MAPPINGS[
                xml_file.parent.name]
