...
flowchart LR
  ...
  subgraph config[Single config — written by setup, not shipped]
...
| work-tracking skill: `BootstrapLabels` tool | Seeds canonical labels into the configured repo |
| work-tracking skill: `RegenerateTasklist` tool | Rebuilds TASKLIST.md from live issues |
| work-tracking skill: `SetWorkRepo` tool | Repo identity setup (existing) — `--bootstrap` extension planned |
