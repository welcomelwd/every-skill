 ...
 * The 8-word description is generated by standard inference (Sonnet).
 */
...
/**
 * Redact secret-shaped strings before any capture is written to disk.
 * Self-contained (no import) so it can't be broken by an upstream module and
 * survives every release layout. Shapes mirror hooks/lib/egress-class-core.ts
 * SECRET_VALUE_SHAPES plus common auth/env/JWT forms. Added for discussion #1365.
 */
const SECRET_SHAPES: RegExp[] = [
  /\bsk-[A-Za-z0-9][A-Za-z0-9_-]{18,}/g,              // OpenAI / Anthropic / OpenRouter sk-* keys
  /\bcsk-[A-Za-z0-9]{16,}/g,                          // Cerebras
  ...
  /\bBearer\s+[A-Za-z0-9._~+/=-]{16,}/gi,             // Authorization: Bearer <token>
  /\beyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}/g, // JWT
  /\b[A-Za-z0-9_]*(?:KEY|TOKEN|SECRET|PASSWORD|PASSWD|CREDENTIAL)[A-Za-z0-9_]*\s*[=:]\s*['"]?[A-Za-z0-9._/+~-]{12,}['"]?/gi, // KEY=value / KEY: value
];

/**
 * Apply every secret shape, replacing matches with [REDACTED].
 */
...
Generate the 8-word description:`;
  ...
  try {
    const result = await inference({
      systemPrompt,
      userPrompt,
      // v6.9.0: medium not low — subprocess cold-start dominates total
      // latency, so Sonnet adds ~1-2s vs Haiku, and the default 30s timeout
      // gives ~3x the headroom of the prior 10s window. Subscription billing
      // makes per-call cost irrelevant.
      level: 'medium',
    });
...
export async function captureFailure(input: FailureCaptureInput): Promise<string | null> {
  ...
  const failuresDir = join(LIFEOS_DIR, 'MEMORY', 'LEARNING', 'FAILURES', yearMonth);
  ...
  // 1. Copy raw transcript — secrets scrubbed before disk (#1365)
  const transcriptDest = join(failureDir, 'transcript.jsonl');
  writeFileSync(transcriptDest, redactSecrets(readFileSync(transcriptPath, 'utf-8')), 'utf-8');
  ...
  writeFileSync(
    ...
    redactSecrets(JSON.stringify(sentimentData, null, 2)),
    'utf-8'
  );
  ...
  writeFileSync(
    ...
    redactSecrets(JSON.stringify(toolCalls, null, 2)),
    'utf-8'
  );
...
`;
  ...
  writeFileSync(join(failureDir, 'CONTEXT.md'), redactSecrets(contextMd), 'utf-8');
