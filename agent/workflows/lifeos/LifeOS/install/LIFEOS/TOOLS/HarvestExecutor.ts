...
// User-specific harvester endpoint. Read from env var so the system file ships
// clean (no hardcoded personal URL). The wrapper (harvest.ts) owns the URL and
// threads it down. If unset, fetchHarvestItems throws a clear error rather than
// crashing on new URL("/items").
...
Usage:
  bun ${path.join(LIFEOS_DIR, "TOOLS", "HarvestExecutor.ts")} [options]
...
async function fetchHarvestItems(token: string, limit: number, itemId?: number): Promise<HarvestItem[]> {
  ...
  // Scope validation to the targeted item. The wrapper always passes --item N,
  // and it fetches a batch of recent items — so one malformed entry sitting in
  // the queue must not poison an unrelated targeted run.
  const targeted =
    itemId !== undefined
      ? record.items.filter(
          (it) => typeof it === "object" && it !== null && (it as Record<string, unknown>).id === itemId,
        )
      : record.items;

  const items = targeted.filter(isHarvestItem);
  if (items.length !== targeted.length) {
    throw new Error(
      itemId !== undefined
        ? `Harvest item ${itemId} is malformed (likely missing classification/confidence) — re-ingest it.`
        : "Harvest API returned one or more malformed items",
    );
  }
...
async function handleCreateKnowledgeIdea(
  ...
  const inferenceResult = await inference({
    systemPrompt:
      "You write concise LifeOS knowledge notes in markdown. Return only the note body, no frontmatter, grounded strictly in the provided harvest metadata.",
    userPrompt: [
      `Title: ${title}`,
      `Source URL: ${item.url}`,
