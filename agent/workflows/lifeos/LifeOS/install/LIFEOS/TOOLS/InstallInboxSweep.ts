...
async function install(): Promise<void> {
  if (!existsSync(TEMPLATE_PATH)) {
    console.error(`[InstallInboxSweep] template missing at ${TEMPLATE_PATH}`);
    process.exit(1);
  }
  const bunPath = await detectBun();
  const bunDir = bunPath.replace(/\/bun$/, "");
  console.log(`[InstallInboxSweep] detected bun at ${bunPath}`);
  ...
  console.log(`[InstallInboxSweep] wrote ${TARGET_PLIST}`);

  const r = await launchctl(["bootstrap", `gui/${u}`, TARGET_PLIST]);
  if (!r.ok) {
    console.error(`[InstallInboxSweep] bootstrap failed: ${r.err.trim()}`);
    process.exit(1);
  }
  console.log(`[InstallInboxSweep] launchd bootstrap OK — ${LABEL} active`);

  const status = await launchctl(["print", `gui/${u}/${LABEL}`]);
  if (status.ok) {
    const stateLine = status.out.split("\n").find((l) => l.includes("state ="));
    console.log(`[InstallInboxSweep] ${stateLine?.trim() ?? "state unknown"}`);
  }
}
...
async function uninstall(): Promise<void> {
  ...
  if (existsSync(TARGET_PLIST)) {
    ...
    console.log(`[InstallInboxSweep] bootout ${r.ok ? "OK" : "FAILED: " + r.err.trim()}`);
    try { unlinkSync(TARGET_PLIST); console.log(`[InstallInboxSweep] removed ${TARGET_PLIST}`); } catch {}
  } else {
    console.log(`[InstallInboxSweep] no plist at ${TARGET_PLIST} — nothing to do`);
  }
}
...
async function status(): Promise<void> {
  ...
  if (!r.ok) {
    console.log(`[InstallInboxSweep] ${LABEL} not loaded`);
    process.exit(1);
  }
  console.log(r.out);
}
...
if (import.meta.main) {
  main().catch((err) => { console.error(`[InstallInboxSweep] Fatal: ${err}`); process.exit(1); });
}
