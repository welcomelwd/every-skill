...
Transform any content into professional LifeOS-themed animations.
...
// - LIFEOS_THEME.typography for text styles
// - LIFEOS_THEME.animation for spring configs
// - LIFEOS_THEME.spacing for layout
```
...
const TitleScene: React.FC<{ title: string; subtitle?: string }> = ({ title, subtitle }) => {
  ...
  const titleScale = spring({ frame, fps, config: LIFEOS_THEME.animation.springDefault })
  const subtitleOpacity = interpolate(frame, [20, 50], [0, 1], { extrapolateRight: 'clamp' })

  return (
    <AbsoluteFill style={{
      backgroundColor: LIFEOS_THEME.colors.background,
      justifyContent: 'center',
      alignItems: 'center',
    }}>
      <h1 style={{
        ...LIFEOS_THEME.typography.title,
        color: LIFEOS_THEME.colors.accent,
      ...
      {subtitle && (
        <p style={{
          ...LIFEOS_THEME.typography.subtitle,
          color: LIFEOS_THEME.colors.textMuted,
...
const KeyPointsScene: React.FC<{ heading: string; points: string[] }> = ({ heading, points }) => {
  ...
  return (
    <AbsoluteFill style={{
      backgroundColor: LIFEOS_THEME.colors.background,
      padding: LIFEOS_THEME.spacing.page,
    }}>
      <h2 style={{
        ...LIFEOS_THEME.typography.heading,
        color: LIFEOS_THEME.colors.text,
        opacity: interpolate(frame, [0, 20], [0, 1], { extrapolateRight: 'clamp' }),
        marginBottom: LIFEOS_THEME.spacing.section,
      }}>
        {heading}
      </h2>

      {points.map((point, i) => {
        const delay = 20 + (i * LIFEOS_THEME.animation.staggerDelay)
        const opacity = interpolate(frame, [delay, delay + 20], [0, 1], { extrapolateRight: 'clamp' })
        const x = interpolate(frame, [delay, delay + 20], [-30, 0], { extrapolateRight: 'clamp' })

        return (
          <div key={i} style={{
            ...LIFEOS_THEME.typography.body,
            color: LIFEOS_THEME.colors.text,
            opacity,
            transform: `translateX(${x}px)`,
            marginBottom: LIFEOS_THEME.spacing.element,
            display: 'flex',
            alignItems: 'flex-start',
          }}>
            <span style={{ color: LIFEOS_THEME.colors.accent, marginRight: 16 }}>✓</span>
            {point}
          </div>
        )
      })}
...
const DataScene: React.FC<{ data: { label: string; value: string }[] }> = ({ data }) => {
  ...
  return (
    <AbsoluteFill style={{
      ...
      alignItems: 'center',
      flexDirection: 'row',
      gap: LIFEOS_THEME.spacing.section,
    }}>
      {data.map((item, i) => {
        const delay = i * 15
        const scale = spring({ frame: Math.max(0, frame - delay), fps, config: LIFEOS_THEME.animation.springBouncy })
...
**Blog post:**
```
User: animate my blog post at ~/path/to/your/blog/example-post.md
```
