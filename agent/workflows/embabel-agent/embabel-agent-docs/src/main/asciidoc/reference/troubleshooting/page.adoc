...
|Ensure you've specified tools using `withToolGroup()` or `withTools()` on your `PromptRunner`. Tools must be explicitly added to each LLM call that needs them. Check that required tool groups are available in your environment.
