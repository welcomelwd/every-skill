...
@SpringBootTest(properties = ["embabel.agent.platform.rest.process-status-enabled=false"])
@ActiveProfiles("test")
@AutoConfigureMockMvc(addFilters = false)
@Import(FakeAiConfiguration::class)
@EnableAutoConfiguration
class ProcessStatusEndpointDisabledTest(
    ...
    @Test
    fun `GET process status is disabled`() {
    ...
    @Test
    fun `AgentProcessStatusController bean is not registered`() {
        assertFalse(applicationContext.getBeansOfType(AgentProcessStatusController::class.java).isNotEmpty())
    }

    @Test
    fun `DELETE process kill remains enabled`() {
        val process = agentPlatform.createAgentProcess(evenMoreEvilWizard(), ProcessOptions(), emptyMap())
