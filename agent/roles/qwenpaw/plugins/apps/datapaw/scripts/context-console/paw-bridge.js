 ...
 * (/api/frontend_plugin/datapaw/files/...), so it can read the host auth
 ...
 * targets the datapaw PawApp backend (/api/datapaw/...).
 *
...
(function () {
  ...
  if (!window.__VITE_AUTH_API_URL__) {
    window.__VITE_AUTH_API_URL__ = "/api/datapaw/context";
  }
  ...
  // The app owns its LLM configuration (same as standalone datapaw-cli);
