...
export function SemanticCatalog({
  ...
  const cliBaseUrl = `${window.location.origin}/api/datapaw/context`;
  const datasourceFlag = datasourceId ? ` --datasource-id ${datasourceId}` : "";
  const cliCommand = `DATAPAW_CM_BASE_URL=${cliBaseUrl} datapaw semantic ${active.command} list${datasourceFlag} --all`;
  ...
  return (
    <section className="datapaw-semantic">
      ...
      <div className="datapaw-semantic-summary" aria-busy={loading}>
        <article>
      ...
      <div className="datapaw-semantic-layout">
        <aside className="datapaw-semantic-nav">
          <span>Resource</span>
        ...
        <div className="datapaw-semantic-table-wrap">
          <div className="datapaw-semantic-table-heading">
            <div>
              <span className="datapaw-eyebrow">{active.singular}</span>
              <h2>{active.label}</h2>
            </div>
            <span>{snapshot?.totals[activeResource] ?? 0} configured</span>
          </div>
          {loading ? (
            <div className="datapaw-semantic-placeholder">
              Loading semantic configuration…
            </div>
          ) : records.length ? (
            <div className="datapaw-semantic-table-scroll">
              <table className="datapaw-semantic-table">
                <thead>
                  <tr>
                    <th>Name</th>
                    <th>Context</th>
      ...
      {snapshot?.weave_tasks.length ? (
        <section className="datapaw-weave-tasks">
          <div>
            <span className="datapaw-eyebrow">Publishing</span>
      ...
      <details className="datapaw-cli-bridge">
