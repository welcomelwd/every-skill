...
from langchain_openviking._uri import UriClassification, classify_uri
...
_LANGGRAPH_IMPORT_ERROR: OptionalDependencyError | None = None
if TYPE_CHECKING:
...
class OpenVikingStore(BaseStore):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self._connection: OpenVikingConnection = OpenVikingConnection(
            client=client,
            url=url,
            api_key=api_key,
            account=account,
        ...
        self.index: bool | list[str] | None = index
        self.wait: bool = wait
        self.timeout: float | None = timeout
        self.search_fetch_limit: int = search_fetch_limit
...
def _extract_uris(value: Any) -> list[str]:
    ...
    if isinstance(value, list):
        nested_uris: list[str] = []
        for item in value:
            nested_uris.extend(_extract_uris(item))
        return nested_uris
    return []
