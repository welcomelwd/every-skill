...
- TypeScript SDK `@openviking/sdk`: for TypeScript and JavaScript HTTP clients.
- OpenCode plugin `@openviking/opencode-plugin`: for OpenCode integration distribution.
- Controlplane MCP `mcp-server-openviking-controlplane`: MCP server and `ov-cp` CLI for the control plane.
...
8. The `20. Release TOS Upload` workflow uploads the source archive and installation scripts to TOS.
