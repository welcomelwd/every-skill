...
You'll need a [Mastra application](/docs) and a [Vercel](https://vercel.com/) account.

:::warning
Vercel Functions use an ephemeral filesystem, so any storage you configure (including observability storage) must be hosted externally. If you're using [LibSQLStore](/integrations/databases/libsql) with a file URL, switch to a remotely hosted database.
:::
...
:::warning
The Vercel deployer doesn't include `flush` calls. If you need this, you'll need to wrap the handler yourself to add the logic before returning the response. Alternatively, deploy to a long-running server like a [virtual machine](/integrations/deploy/digital-ocean) where this isn't an issue.
:::
