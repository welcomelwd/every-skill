...
Photon delivers to public HTTPS endpoints only. It won't deliver to `http://`, to private addresses like `localhost`, or through a redirect. For local development, use a tunnel as described in the [Channels overview](/docs/capabilities/channels#webhook-routes).
...
On serverless platforms, prefer webhooks. A gateway listener needs a process that stays alive. See [Serverless deployment](/docs/capabilities/channels#serverless-deployment).
