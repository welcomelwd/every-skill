...
Memory results will be stored in one or more of your configured [storage providers](/docs/storage).
...
<Steps>
  ...
  <StepItem>
    ...
    For more details on available providers and how storage works in Mastra, visit the [storage](/docs/storage) documentation.
  </StepItem>
...
The supervisor forwards its conversation context to the subagent so it has enough background to complete the task. Only the delegation prompt and the subagent's response are saved, the full parent conversation isn't stored. You can control which messages reach the subagent with the [`messageFilter`](/docs/subagents#message-filtering) callback.
