...
A parent agent that coordinates subagents is often called a supervisor. The supervisor pattern is one approach to building multi-agent systems in Mastra. For other patterns, read the [conceptual overview](/docs/guides/multi-agent-systems).
...
Subagent invocations are dispatched as tool calls, so they can run as [background tasks](/docs/harness/background-tasks). This is useful when one or more delegations are long-running and you don't want them to block the parent agent's response.
...
If a subagent isn't listed under the parent agent's `backgroundTasks.tools` but has its own background-eligible tools, the parent agent still dispatches the subagent as a background task and inherits its config. See [Inheriting from the subagent](/docs/harness/background-tasks#inheriting-from-the-subagent) for details.

## Subagent versioning

When using the [editor](/docs/studio/editor), you can control which stored version of each subagent the parent agent uses at runtime. Set version overrides on the Mastra instance or per invocation:
...
- [Background tasks](/docs/harness/background-tasks)
...
- [Concept: Multi-agent systems](/docs/guides/multi-agent-systems)
