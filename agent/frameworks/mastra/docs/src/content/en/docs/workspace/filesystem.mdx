...
- [`S3Filesystem`](/integrations/file-storage/amazon-s3): Stores files in Amazon S3 or S3-compatible storage (R2, MinIO, Tigris)
- [`GCSFilesystem`](/integrations/file-storage/google-cloud-storage): Stores files in Google Cloud Storage
- [`PlatformFilesystem`](/reference/workspace/platform-filesystem): Stores files in a Mastra Platform workspace bucket
- [`GoogleDriveFilesystem`](/integrations/file-storage/google-drive): Stores files inside a Google Drive folder
- [`AzureBlobFilesystem`](/integrations/file-storage/azure-blob): Stores files in Azure Blob Storage
- [`FilesSDKFilesystem`](/integrations/file-storage/vercel-files): Stores files in any [FilesSDK](https://files-sdk.dev) adapter (S3, R2, GCS, Azure Blob, Vercel Blob, local filesystem, and more): useful when you want one provider that can target multiple backends
- [`AgentFSFilesystem`](/integrations/file-storage/agentfs): Stores files in a Turso/SQLite database via AgentFS
- [`MesaFilesystem`](/integrations/file-storage/mesa): Stores files in versioned Mesa repos
- [`ArchilFilesystem`](/integrations/file-storage/archil): Stores files on Archil elastic, serverless disks
...
- [GoogleDriveFilesystem reference](/integrations/file-storage/google-drive)
- [AzureBlobFilesystem reference](/integrations/file-storage/azure-blob)
- [FilesSDKFilesystem reference](/integrations/file-storage/vercel-files)
- [AgentFSFilesystem reference](/integrations/file-storage/agentfs)
- [MesaFilesystem reference](/integrations/file-storage/mesa)
