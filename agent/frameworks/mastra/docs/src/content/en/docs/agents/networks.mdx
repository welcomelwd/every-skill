...
Agent networks are deprecated and will be removed in a future major release. [Supervisor agents](../subagents) using `agent.stream()` or `agent.generate()` are now the recommended approach. It provides the same multi-agent coordination with better control, a simpler API, and easier debugging.

See the [migration guide](/reference/migrations/network-to-supervisor) to upgrade.
...
Network approval uses snapshots to capture execution state. Ensure a [storage provider](/docs/storage) is enabled in your Mastra instance.
...
- [Migration: `.network()` to supervisor agents](/reference/migrations/network-to-supervisor)
