...
Notification inbox storage is available in the storage adapters that support richer memory and signal workflows: [libSQL](/integrations/databases/libsql), [PostgreSQL](/integrations/databases/postgresql), and [MongoDB](/integrations/databases/mongodb). These adapters expose notification records through `getStore('notifications')`.
