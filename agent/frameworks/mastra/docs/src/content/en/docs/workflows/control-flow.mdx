...
```mermaid
flowchart LR
  accTitle: Running workflow steps in sequence
  accDescr: The workflow passes its input to step1, then passes step1's output to step2 before completing.
  start(( start )) -- in --> step1([step1])
  step1 -- out --> step2([step2])
  step2 -- out --> stop(( end ))
...
```mermaid
flowchart LR
  accTitle: Running workflow steps in parallel
  accDescr: The workflow runs step1 and step2 in parallel. Their outputs join at step3 before the workflow completes.
  start(( start )) -- in --> step1([step1])
  start -- in --> step2([step2])
  step1 -- out --> step3([step3])
  step2 -- out --> step3
  step3 -- out --> stop(( end ))
