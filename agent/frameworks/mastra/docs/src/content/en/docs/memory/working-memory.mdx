...
By default, working memory reaches the model as part of the system message. You can opt into delivering it as a [state signal](/docs/harness/signals) instead by setting `useStateSignals: true`:
