...
Use [`createRoute()`](/reference/server/create-route) in `apiRoutes` to parse and validate path and query parameters, along with request bodies, using Zod. The schemas also infer the handler parameters and generate OpenAPI metadata.
...
For more information about authentication providers, see the [Auth documentation](/docs/auth/overview).
...
- [Mastra Server](/docs/server/overview): Server configuration options
