...
After registration, retrieve your agent with [`mastra.getAgentById()`](/reference/core/getAgentById). Call `.generate()` for a complete response or `.stream()` to deliver tokens in real time. You can call agents from [workflow steps](/docs/workflows/agents-and-tools), [tools](/docs/agents/tools), the [Mastra Client](/reference/client-js/mastra-client), route handlers, [server adapters](/docs/server/server-adapters), or the command line. See [Integrations](/integrations) to use agents in your application framework.
...
| Goal | Start here |
| - | - |
| Give your agent tools to call external APIs or services | [Tools](/docs/agents/tools) |
...
| Human-in-the-loop: Pause execution and wait for human approval | [Human-in-the-loop](/docs/agents/human-in-the-loop) |
| Build a multi-agent network | [Supervisor agents](/docs/subagents) |
| Register subagents | [Tools](/docs/agents/tools#agents-as-tools) |
...
| Build agents that correct their work | [Rubric scorer](/docs/subagents#rubric-scorer) |
| Swap instructions or models based on request context | [Dynamic configuration](/docs/server/request-context) |
| Add speech-to-text or text-to-speech | [Voice](/reference/voice/overview) |
| Connect to Slack, Discord, or Telegram | [Channels](/docs/channels) |
...
Read the [conceptual overview of multi-agent systems](/docs/guides/multi-agent-systems) to learn how you can apply different patterns with Mastra.
