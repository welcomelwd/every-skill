...
A signal provider monitors an external source, such as GitHub, Slack, continuous integration (CI), or your own API, and pushes [notification signals](/docs/harness/signals#notification-signals) into subscribed agent threads.
...
A signal provider is the producing side of the [signals](/docs/harness/signals) system. It brings external events into a thread, while the signal APIs control how the thread consumes them.
...
:::note
Notification delivery requires a storage adapter with notification support, such as [libSQL](/integrations/databases/libsql), [PostgreSQL](/integrations/databases/postgresql), or [MongoDB](/integrations/databases/mongodb). Configure storage on the Mastra instance so `notify()` can store notification records.
:::
...
:::note
For a complete polling-provider build with notification storage, agent registration, thread subscription, and testing, follow [Building a signal provider](https://mastra.ai/blog/building-a-signal-provider).
:::
...
For a production provider that watches GitHub pull requests, see the [GitHub Channels integration](/integrations/channels/github).

## Related

- [Guide: Building a signal provider](https://mastra.ai/blog/building-a-signal-provider)
- [Signals](/docs/harness/signals)
- [Notification signals](/docs/harness/signals#notification-signals)
