    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'object',
            description:
              "Step execution result with reason, warnings, and continuation info. `rawReason` holds the provider's own finish reason when it reports one.",
          },
          {
            name: 'output',
            type: 'object',
        ...
        parameters: [
          {
            ...
            type: 'object',
            description:
              'Step execution result. `reason` holds the normalized finish reason, and `rawReason` holds the provider\'s own finish reason when it reports one. A `reason` of `"error"` means the run failed.',
          },
          {
            name: 'output',
            type: 'object',
...
Emitted when a tool call is dispatched as a [background task](/docs/harness/background-tasks) and `streamUntilIdle()` is used.
...
Emitted on every evaluation of an agent [goal](/docs/harness/goals). Consumers use this to render judge progress and the result mid-run. A goal that has no judge model configured produces no `goal` chunk.
