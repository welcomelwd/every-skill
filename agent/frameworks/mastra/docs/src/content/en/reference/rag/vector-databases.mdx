...
// Create an index with dimension 1536 (for __AI_SDK_OPENAI_EMBEDDING_MODEL__)
await store.createIndex({
  indexName: 'myCollection',
  dimension: 1536,
})
...
- OpenAI `__AI_SDK_OPENAI_EMBEDDING_MODEL__`: 1536 dimensions (or custom, e.g., 256)
...
- Match embedding dimensions to your model (e.g., 1536 for `__AI_SDK_OPENAI_EMBEDDING_MODEL__`)
