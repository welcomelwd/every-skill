...
Agent networks are deprecated and will be removed in a future release. Use [supervisor agents](/docs/capabilities/subagents) with `agent.stream()` or `agent.generate()` instead. See the [migration guide](/reference/migrations/network-to-supervisor) to upgrade.
