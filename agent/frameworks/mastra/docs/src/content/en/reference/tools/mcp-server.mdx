...
<PropertiesTable
  ...
  },
  {
    ...
    type: "'2025-11-25' | '2026-07-28'",
    isOptional: true,
    description:
      "Opt-in MCP protocol revision. Omitted (or '2025-11-25') keeps the legacy behavior exactly. Set to '2026-07-28' to serve the stateless MCP revision. See the Protocol versions section for details.",
  },
  {
    name: 'cacheHints',
    ...
    isOptional: true,
    description:
      "Cache hints (`ttlMs` / `cacheScope`) advertised on cacheable results of the '2026-07-28' protocol revision, keyed by operation (e.g. 'tools/list'). Only applied when `protocolVersion: '2026-07-28'` is set.",
  },
]}
/>
...
:::warning
The HTTP+SSE transport is deprecated in the MCP specification. Use `startHTTP()` (streamable HTTP) instead.
:::
...
:::note[When to use serverless: true]
