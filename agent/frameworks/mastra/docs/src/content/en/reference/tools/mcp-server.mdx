...
For a practical example of packaging a stdio server, see [Publish a stdio server package](/docs/mcp/overview#publish-a-stdio-server-package).
...
Link a tool to its app resource by setting `mcp._meta.ui.resourceUri` in `createTool()` to the matching `ui://` URI. The server normalizes this metadata for older hosts when listing tools. Visit [MCP Apps](/docs/mcp/overview) for the full app bridge API and usage patterns.
