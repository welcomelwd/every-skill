...
Related providers: [`S3Filesystem`](/integrations/file-storage/amazon-s3) for direct S3 access, [`LocalFilesystem`](/reference/workspace/local-filesystem) for local directories.
