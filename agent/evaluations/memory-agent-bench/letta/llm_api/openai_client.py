...
class OpenAIClient(LLMClientBase):
    ...
    def build_request_data(
        ...
        data = ChatCompletionRequest(
            model=model,
            messages=[self.load_image_content_from_database(m) for m in openai_message_list],
    ...
    def load_image_content_from_database(self, message):
        """
        Converts image URIs in the message to base64 format.
        """
        # Only user message can have images.
        ...
        for m in message.content:
            if m['type'] == 'image_url':
                message_content.append({
                    'type': 'image_url',
                    'image_url': {'url': self.image_manager.get_image_by_id(m['image_id']).image_data, 'detail': m['detail']},
                })
            else:
