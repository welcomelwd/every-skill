...
class LocalClient(AbstractClient):
    ...
    def send_message(
        ...
        Args:
            message (str|list[dict]): Message to send
            role (str): Role of the message
            agent_id (str): ID of the agent
            name(str): Name of the sender
            stream (bool): Stream the response (default: `False`)
        ...
        if isinstance(message, list):
            # convert dict to MessageCreate
            def convert_message(m):
                if m['type'] == 'text':
                    return TextContent(**m)
                elif m['type'] == 'image_url':
                    # insert the image into the database
                    image = self.server.image_manager.insert_image(
                        image_data=m['image_url']['url'],
                        actor=self.user
                    )
                    return ImageContent(
                        image_id=image.id,
            ...
            message = [convert_message(m) for m in message]
