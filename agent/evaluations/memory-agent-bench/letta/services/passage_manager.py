...
class PassageManager:
    ...
    def clear_all_passages(self, actor: PydanticUser) -> bool:
        """Clear all passages from the database."""
        with self.session_maker() as session:
            # list all ids
            source_passages = session.query(SourcePassage).all()
            agent_passages = session.query(AgentPassage).all()
            source_ids = [p.id for p in source_passages]
            agent_ids = [p.id for p in agent_passages]
            for passage_id in source_ids:
                self.delete_passage_by_id(passage_id=passage_id, actor=actor)
            for passage_id in agent_ids:
                self.delete_passage_by_id(passage_id=passage_id, actor=actor)
        return True
