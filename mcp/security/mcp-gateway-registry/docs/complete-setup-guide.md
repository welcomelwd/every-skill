...
### Required Secrets and Production Hardening

The steps above cover the minimum to boot a local Keycloak stack. This subsection lists the parameters that a deployment either **cannot start without** (fail-closed) or **should set for a secure production posture**. The full cross-surface reference (Docker / Terraform / Helm names) is in [`docs/unified-parameter-reference.md`](unified-parameter-reference.md).

> On the docker-compose path, `build_and_run.sh` auto-generates the fail-closed secrets (`SECRET_KEY`, `AUTH_SERVER_NGINX_MARKER_SECRET`, `DOCUMENTDB_PASSWORD`, `METRICS_KEY_PEPPER`) into `.env` when they are left empty, and on Terraform/ECS they are generated into AWS Secrets Manager. Set them explicitly only if you are managing secrets yourself. The table still lists them so you know they exist and why they matter.

#### Fail-closed secrets (a service refuses to start, or a surface is disabled, without them)

| Parameter | Consequence if unset | Notes |
|-----------|----------------------|-------|
| `SECRET_KEY` | Registry + auth-server refuse to start. | JWT/session signing and backend MCP-credential encryption key; must be `>= 32` chars and not a placeholder. Auto-generated by `build_and_run.sh`. |
| `AUTH_SERVER_NGINX_MARKER_SECRET` | Registry + auth-server refuse to start. | Shared secret nginx sets so the auth-server only mints proxy tokens for genuine `/validate` subrequests; prevents a forged `/validate` bypass. Must be **identical** across registry and auth-server. Auto-generated. |
| `DOCUMENTDB_PASSWORD` | Datastore container won't boot (`${...:?}`). | Admin password for the MongoDB/DocumentDB datastore (default `mongodb-ce` backend). Auto-generated. |
| `METRICS_KEY_PEPPER` | The metrics-service refuses to start. | Per-deployment HMAC pepper for stored metrics API-key hashes; `>= 32` chars, not a placeholder. Auto-generated. |
| `OPENBAO_TOKEN` | Compose won't start (`${OPENBAO_TOKEN:?}`) when the egress vault uses the OpenBao backend with token auth. | Only needed for `SECRET_STORE_BACKEND=openbao` + `OPENBAO_AUTH_METHOD=token`; EKS uses Kubernetes auth instead. |
| `METRICS_ADMIN_API_KEY` | metrics-service `/admin/*` returns `503` until set (ingest + daily cleanup still work). | Privilege-separated admin credential; `>= 32` chars and **distinct from any ingest key**. docker-compose upgraders must add it; Terraform/CDK auto-generate it. A startup log announces whether the admin surface is enabled. |
| `<selected IdP> client secret` | Login for that provider fails. | Only the active provider's credentials are required (e.g. `KEYCLOAK_CLIENT_SECRET` + `KEYCLOAK_M2M_CLIENT_SECRET`, or the `ENTRA_*` / `OKTA_*` / `AUTH0_*` / `COGNITO_*` / `PINGFEDERATE_*` set). See `unified-parameter-reference.md` Group 12. |

#### Production-hardening parameters (secure posture; insecure or degraded default if left unset)

| Parameter | Why set it | Default without it |
|-----------|------------|--------------------|
| `SESSION_COOKIE_SECURE=true` | Required for any HTTPS/production deployment. | `false` in the example (local HTTP only). |
| `TRUSTED_REAL_IP_CIDRS` | Behind an ALB/CloudFront, lets nginx recover the real client IP. | Audit client-IP and per-IP rate limits collapse to the load balancer's IP. Terraform auto-sets it to the VPC CIDR. |
| `TRUSTED_PROXY_HOPS` | Must match your reverse-proxy depth (e.g. `2` for CloudFront + ALB). | `1`; wrong depth means the trusted-hop selection is wrong. |
| `OAUTH2_ALLOWED_REDIRECT_URIS` | Exact-match allowlist of login/logout redirect URIs (open-redirect defense). | Falls back to a weaker cookie-domain heuristic that can admit attacker-controlled subdomains. |
| `CORS_ALLOWED_ORIGINS` | Set when a separate browser frontend calls the API. | Same-origin only (fail-closed, no wildcard). |
