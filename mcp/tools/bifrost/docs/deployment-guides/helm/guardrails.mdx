...
<Tab title="Lakera Guard">

Calls Lakera Guard's `POST /v2/guard` endpoint with an OpenAI-style conversation. The Lakera project selects the policy; Bifrost controls whether a flagged result blocks, is recorded, or is redacted from supported payload spans.
...
`base_url` is optional and defaults to `https://api.lakera.ai`. For `action: "redact"`, Lakera supplies supported message-content spans and Bifrost performs the transformation. See [Lakera Guard](/integrations/guardrails/lakera) for request compatibility and fail-closed behavior.
...
| Field | Required | env.\* supported | Notes |
|-------|----------|-----------------|-------|
| `api_key` | Yes | Yes | Lakera API key |
| `project_id` | Yes | **Plain only** | Lakera project ID whose assigned policy controls screening |
| `base_url` | No | Yes | Lakera API base. Defaults to `https://api.lakera.ai` |
