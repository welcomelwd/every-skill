 ...
 */
export async function expectScrollbarGutterIdleHidden(
  ...
  await expect(scrollbars.length).toBeGreaterThan(0);
  for (const bar of scrollbars) {
    await expect(bar.getAttribute("data-state")).toBe("hidden");
  }
}
