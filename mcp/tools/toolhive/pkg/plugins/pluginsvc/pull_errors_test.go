...
func TestClassifyPullError(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			err:  newPluginErrResp(http.StatusUnauthorized),
			want: http.StatusUnauthorized,
		},
		{
			name: "registry 403 maps to 401",
			err:  newPluginErrResp(http.StatusForbidden),
			want: http.StatusUnauthorized,
		},
		{
			name: "registry 404 maps to 404",
			err:  newPluginErrResp(http.StatusNotFound),
			want: http.StatusNotFound,
		},
		{
			name: "registry 429 maps to 429",
			err:  newPluginErrResp(http.StatusTooManyRequests),
			want: http.StatusTooManyRequests,
		},
		{
			name: "registry 400 maps to 502",
			err:  newPluginErrResp(http.StatusBadRequest),
			want: http.StatusBadGateway,
		},
		{
			name: "registry 500 maps to 502",
			err:  newPluginErrResp(http.StatusInternalServerError),
			want: http.StatusBadGateway,
		},
		{
			name: "registry 503 maps to 502",
			err:  newPluginErrResp(http.StatusServiceUnavailable),
			want: http.StatusBadGateway,
		},
		{
			name: "wrapped registry 401 maps to 401",
			err:  fmt.Errorf("copy graph: %w", newPluginErrResp(http.StatusUnauthorized)),
			want: http.StatusUnauthorized,
		},
		{
		...
		},
		{
			...
			err: fmt.Errorf("pulling from registry: %w",
				fmt.Errorf("wrapped: %w", newPluginErrResp(http.StatusNotFound))),
			want: http.StatusNotFound,
		},
	}
...
// newPluginErrResp constructs a *errcode.ErrorResponse with the given HTTP
// status for use as a synthetic oras-go error. Mirrors skillsvc.newErrResp.
func newPluginErrResp(status int) *errcode.ErrorResponse {
