...
//
//	mockgen -destination=mocks/mock_storage.go -package=mocks -source=types.go Storage,PendingAuthorizationStorage,AssertionJWTConsumer,ClientRegistry,UpstreamTokenStorage,UpstreamTokenRefresher,UserStorage,DCRCredentialStore
//
...
// MockAssertionJWTConsumer is a mock of AssertionJWTConsumer interface.
type MockAssertionJWTConsumer struct {
	ctrl     *gomock.Controller
	recorder *MockAssertionJWTConsumerMockRecorder
	isgomock struct{}
}

// MockAssertionJWTConsumerMockRecorder is the mock recorder for MockAssertionJWTConsumer.
type MockAssertionJWTConsumerMockRecorder struct {
	mock *MockAssertionJWTConsumer
}

// NewMockAssertionJWTConsumer creates a new mock instance.
func NewMockAssertionJWTConsumer(ctrl *gomock.Controller) *MockAssertionJWTConsumer {
	mock := &MockAssertionJWTConsumer{ctrl: ctrl}
	mock.recorder = &MockAssertionJWTConsumerMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use.
func (m *MockAssertionJWTConsumer) EXPECT() *MockAssertionJWTConsumerMockRecorder {
	return m.recorder
}

// ConsumeAssertionJWT mocks base method.
func (m *MockAssertionJWTConsumer) ConsumeAssertionJWT(ctx context.Context, purpose, issuer, jti string, exp time.Time) error {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "ConsumeAssertionJWT", ctx, purpose, issuer, jti, exp)
	ret0, _ := ret[0].(error)
	return ret0
}

// ConsumeAssertionJWT indicates an expected call of ConsumeAssertionJWT.
func (mr *MockAssertionJWTConsumerMockRecorder) ConsumeAssertionJWT(ctx, purpose, issuer, jti, exp any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "ConsumeAssertionJWT", reflect.TypeOf((*MockAssertionJWTConsumer)(nil).ConsumeAssertionJWT), ctx, purpose, issuer, jti, exp)
}
