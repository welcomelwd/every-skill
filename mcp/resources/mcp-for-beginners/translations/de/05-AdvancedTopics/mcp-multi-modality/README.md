...
Multi-modale Anwendungen gewinnen in der KI zunehmend an Bedeutung, da sie reichhaltigere Interaktionen und komplexere Aufgaben ermöglichen. Das Model Context Protocol (MCP) bietet einen Rahmen zum Erstellen von multi-modalen Anwendungen, die verschiedene Datentypen wie Text, Bilder und Audio verarbeiten können.

MCP unterstützt nicht nur textbasierte Interaktionen, sondern auch multi-modale Funktionen, die es Modellen erlauben, mit Bildern, Audio und anderen Datentypen zu arbeiten.

## Einführung

In dieser Lektion lernst du, wie man eine multi-modale Anwendung erstellt.

## Lernziele

Am Ende dieser Lektion wirst du in der Lage sein:

- Multi-modale Optionen zu verstehen
- Eine multi-modale App zu implementieren.

## Architektur für Multi-Modal-Unterstützung

Multi-modale MCP-Implementierungen umfassen typischerweise:

- **Modal-spezifische Parser**: Komponenten, die verschiedene Medientypen in Formate umwandeln, die das Modell verarbeiten kann.
- **Modal-spezifische Tools**: Spezielle Werkzeuge, die für bestimmte Modalitäten entwickelt wurden (Bildanalyse, Audioverarbeitung)
- **Einheitliches Kontextmanagement**: System zur Aufrechterhaltung des Kontexts über verschiedene Modalitäten hinweg
- **Antwortgenerierung**: Fähigkeit, Antworten zu erzeugen, die mehrere Modalitäten enthalten können.

## Multi-Modal Beispiel: Bildanalyse

Im folgenden Beispiel analysieren wir ein Bild und extrahieren Informationen.
...
- Ein `ImageAnalysisTool` erstellt, das Bilder mit einem hypothetischen `IImageAnalysisService` analysieren kann.
- Den MCP-Server so konfiguriert, dass er größere Anfragen und Bildinhalte unterstützt.
- Das Bildanalyse-Tool beim Server registriert.
- Eine Methode implementiert, um Bilder von einer URL herunterzuladen und sie basierend auf dem angeforderten Typ (Objekte, Text, Gesichter usw.) zu analysieren.
- Strukturierte Ergebnisse in einem mit der MCP-Spezifikation konformen Format zurückgegeben.

## Multi-Modal Beispiel: Audioverarbeitung

Audioverarbeitung ist eine weitere häufige Modalität in multi-modalen Anwendungen. Unten findest du ein Beispiel, wie man ein Tool zur Audio-Transkription implementiert, das Audiodateien verarbeiten und Transkriptionen zurückgeben kann.
...
- Ein `AudioTranscriptionTool` erstellt, das Audiodateien transkribieren kann.
- Das Schema des Tools definiert, um entweder eine URL oder base64-kodierte Audiodaten zu akzeptieren.
- Die `execute`-Methode implementiert, um die Audioverarbeitung und Transkription durchzuführen.
- Den MCP-Server so konfiguriert, dass er multi-modale Anfragen einschließlich Audio- und Bildverarbeitung unterstützt.
- Das Audio-Transkriptions-Tool beim Server registriert.
- Eine Methode implementiert, um Audiodateien von einer URL herunterzuladen oder base64-Audiodaten zu dekodieren.
- Einen `AudioProcessor`-Dienst verwendet, um die eigentliche Transkriptionslogik zu übernehmen.
- Den MCP-Server gestartet, um auf Anfragen zu hören.

### Multi-Modal Beispiel: Multi-Modale Antwortgenerierung
