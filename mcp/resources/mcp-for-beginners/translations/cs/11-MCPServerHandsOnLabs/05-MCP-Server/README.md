# Implementace MCP serveru

## 🎯 Co tento workshop pokrývá

Tento praktický workshop vás provede implementací produkčně připraveného MCP serveru pomocí frameworku FastMCP. Vytvoříte základní strukturu serveru, integraci s databází, nástroje pro přístup k datům a základy pro analytiku maloobchodu poháněnou AI.

## Přehled

MCP server je srdcem našeho řešení pro analytiku maloobchodu. Funguje jako most mezi AI asistenty a databází PostgreSQL, poskytuje bezpečný a inteligentní přístup k obchodním datům prostřednictvím standardizovaného protokolu.

Tento workshop vás naučí, jak vytvořit robustní a škálovatelný MCP server podle podnikových vzorů a osvědčených postupů.

## Cíle učení

Na konci tohoto workshopu budete schopni:

- **Vytvořit** FastMCP server s vhodnou architekturou a organizací
- **Implementovat** integraci databáze s poolingem připojení a zpracováním chyb
- **Vytvořit** MCP nástroje pro introspekci schématu databáze a provádění dotazů
- **Konfigurovat** správu kontextu Row Level Security
- **Přidat** funkce monitorování zdraví a observability
- **Otestovat** implementaci MCP serveru lokálně a pomocí VS Code

## 📁 Struktura projektu

Podívejme se na organizaci MCP serveru:
...
### Konfigurace prostředí (`config.py`)

Nejprve vytvoříme robustní systém konfigurace:
...
### Klíčové vlastnosti konfigurace

- **Načítání proměnných prostředí**: Automatická podpora souborů .env
- **Typová bezpečnost**: Validace datových tříd a typové nápovědy
- **Flexibilní výchozí hodnoty**: Smysluplné výchozí hodnoty pro vývoj
- **Validace**: Validace konfigurace s užitečnými chybovými zprávami
- **Bezpečnost**: Citlivé hodnoty pouze z proměnných prostředí

## 🗄️ Vrstva integrace databáze

### Poskytovatel PostgreSQL (`sales_analysis_postgres.py`)

Implementujeme vrstvu integrace databáze:
...
### Klíčové vlastnosti vrstvy databáze

- **Pooling připojení**: Efektivní správa zdrojů pomocí asyncpg
- **Integrace RLS**: Automatické nastavení kontextu Row Level Security
- **Introspekce schématu**: Dynamické zjišťování schématu tabulek
- **Zpracování chyb**: Komplexní správa chyb a logování
- **Formátování dotazů**: Výsledky přátelské pro AI
- **Monitorování zdraví**: Kontrola konektivity databáze a stavu poolu

## 🔧 Hlavní implementace MCP serveru

### FastMCP server (`sales_analysis.py`)

Nyní implementujeme hlavní MCP server:
...
### Klíčové vlastnosti MCP serveru

- **Registrace nástrojů**: Deklarativní definice nástrojů s typovou bezpečností
- **Správa kontextu RLS**: Automatická extrakce identity uživatele a nastavení kontextu
- **Zpracování chyb**: Komplexní správa chyb s uživatelsky přívětivými zprávami
- **Správa životního cyklu**: Správné spuštění/zastavení s čištěním zdrojů
- **Monitorování zdraví**: Vestavěné endpointy pro kontrolu zdraví
- **Podpora vývoje**: Hot reload a možnosti ladění

## 🏥 Monitorování zdraví

### Implementace kontroly zdraví (`health_check.py`)
...
### Testování integrace ve VS Code

1. **Konfigurujte VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testujte v AI chatu**:
   - Otevřete AI chat ve VS Code
   - Zadejte `#zava` a vyberte svůj server
   - Zeptejte se: "Jaké tabulky jsou dostupné?"
   - Zeptejte se: "Ukaž mi 5 nejlepších obchodů podle počtu objednávek"

### Jednotkové testování

Vytvořte komplexní jednotkové testy:
...
✅ **Funkční MCP server**: FastMCP server s integrací databáze  
✅ **Správu konfigurace**: Robustní konfiguraci založenou na prostředí  
✅ **Vrstva databáze**: Integrace PostgreSQL s poolingem připojení  
✅ **MCP nástroje**: Nástroje pro introspekci schématu a provádění dotazů  
✅ **Integrace RLS**: Správa kontextu Row Level Security  
✅ **Monitorování zdraví**: Komplexní endpointy pro kontrolu zdraví  
✅ **Strategii testování**: Lokální testování a integrace ve VS Code  

## 🚀 Co dál

Pokračujte s **[Lab 06: Vývoj nástrojů](../06-Tools/README.md)**, kde:

- Rozšíříte svou kolekci MCP nástrojů
- Implementujete pokročilé vzory dotazů
- Přidáte validaci a transformaci dat
- Vytvoříte specializované analytické nástroje

## 📚 Další zdroje

### Framework FastMCP
- [Dokumentace FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Oficiální průvodce FastMCP
- [Specifikace MCP](https://modelcontextprotocol.io/docs/) - Specifikace protokolu
- [Průvodce vývojem nástrojů](https://modelcontextprotocol.io/docs/tools/) - Tvorba MCP nástrojů

### Integrace databáze
- [Dokumentace asyncpg](https://magicstack.github.io/asyncpg/current/) - Asynchronní ovladač PostgreSQL
- [Nejlepší praktiky poolingu připojení](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Ladění PostgreSQL
- [Průvodce Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementace RLS

### Vzory FastAPI
- [Dokumentace FastAPI](https://fastapi.tiangolo.com/) - Referenční příručka webového frameworku
- [Dependency Injection](https://fastapi.tiangolo.com/tutorial/dependencies/) - Vzory FastAPI
- [Background Tasks](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Správa asynchronních úloh

---

**Další krok**: Připraveni rozšířit své nástroje? Pokračujte s [Lab 06: Vývoj nástrojů](../06-Tools/README.md)
