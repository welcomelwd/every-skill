...
هذا المختبر العملي يوجهك خلال عملية تنفيذ خادم MCP جاهز للإنتاج باستخدام إطار عمل FastMCP. ستقوم ببناء الهيكل الأساسي للخادم، تنفيذ تكامل قاعدة البيانات، إنشاء أدوات للوصول إلى البيانات، ووضع الأساس لتحليلات التجزئة المدعومة بالذكاء الاصطناعي.

## نظرة عامة

خادم MCP هو قلب حل تحليلات التجزئة لدينا. يعمل كجسر بين مساعدي الذكاء الاصطناعي وقاعدة بيانات PostgreSQL، مما يوفر وصولًا آمنًا وذكيًا إلى بيانات الأعمال عبر بروتوكول موحد.

هذا المختبر يعلمك كيفية بناء خادم MCP قوي وقابل للتوسع وفقًا لأنماط المؤسسات وأفضل الممارسات.

## أهداف التعلم

بنهاية هذا المختبر، ستكون قادرًا على:

- **بناء** خادم FastMCP مع هيكل وتنظيم مناسبين  
- **تنفيذ** تكامل قاعدة البيانات مع إدارة تجمع الاتصالات ومعالجة الأخطاء  
- **إنشاء** أدوات MCP لاستكشاف مخطط قاعدة البيانات وتنفيذ الاستعلامات  
- **تكوين** إدارة سياق أمان مستوى الصفوف (Row Level Security)  
- **إضافة** ميزات مراقبة الصحة وقابلية الملاحظة  
- **اختبار** تنفيذ خادم MCP محليًا ومع VS Code  

## 📁 هيكل المشروع

لنستعرض تنظيم خادم MCP:
...
### تكوين البيئة (`config.py`)

أولاً، لنقم بإنشاء نظام تكوين قوي:
...
- **تحميل متغيرات البيئة**: دعم تلقائي لملفات .env  
- **سلامة النوع**: التحقق من صحة dataclass وتلميحات النوع  
- **افتراضيات مرنة**: افتراضيات معقولة للتطوير  
- **التحقق**: التحقق من التكوين مع رسائل خطأ مفيدة  
- **الأمان**: القيم الحساسة فقط من متغيرات البيئة  

## 🗄️ طبقة تكامل قاعدة البيانات

### موفر PostgreSQL (`sales_analysis_postgres.py`)

لنقم بتنفيذ طبقة تكامل قاعدة البيانات:
...
### ميزات طبقة قاعدة البيانات الرئيسية

- **إدارة تجمع الاتصالات**: إدارة الموارد بكفاءة باستخدام asyncpg  
- **تكامل RLS**: إعداد سياق أمان مستوى الصفوف تلقائيًا  
- **استكشاف المخطط**: اكتشاف مخطط الجدول ديناميكيًا  
- **معالجة الأخطاء**: إدارة شاملة للأخطاء وتسجيلها  
- **تنسيق الاستعلامات**: تنسيق النتائج بطريقة ملائمة للذكاء الاصطناعي  
- **مراقبة الصحة**: التحقق من اتصال قاعدة البيانات وحالة التجمع  

## 🔧 تنفيذ خادم MCP الرئيسي

### خادم FastMCP (`sales_analysis.py`)

الآن لنقم بتنفيذ خادم MCP الرئيسي:
...
### ميزات خادم MCP الرئيسية

- **تسجيل الأدوات**: تعريف الأدوات بشكل تصريحي مع سلامة النوع  
- **إدارة سياق RLS**: استخراج هوية المستخدم وإعداد السياق تلقائيًا  
- **معالجة الأخطاء**: إدارة شاملة للأخطاء مع رسائل مفيدة للمستخدم  
- **إدارة دورة الحياة**: بدء/إيقاف صحيح مع تنظيف الموارد  
- **مراقبة الصحة**: نقاط نهاية للتحقق من الصحة مدمجة  
- **دعم التطوير**: إعادة تحميل ساخن وقدرات تصحيح الأخطاء  

## 🏥 مراقبة الصحة

### تنفيذ التحقق من الصحة (`health_check.py`)
...
2. **اختبار في دردشة الذكاء الاصطناعي**:  
   - افتح دردشة الذكاء الاصطناعي في VS Code  
   - اكتب `#zava` واختر خادمك  
   - اسأل: "ما الجداول المتاحة؟"  
   - اسأل: "أرني أفضل 5 متاجر حسب عدد الطلبات"  

### اختبار الوحدة

قم بإنشاء اختبارات وحدة شاملة:
...
✅ **خادم MCP عامل**: خادم FastMCP مع تكامل قاعدة البيانات  
✅ **إدارة التكوين**: نظام تكوين قوي يعتمد على البيئة  
✅ **طبقة قاعدة البيانات**: تكامل PostgreSQL مع إدارة تجمع الاتصالات  
✅ **أدوات MCP**: أدوات لاستكشاف المخطط وتنفيذ الاستعلامات  
✅ **تكامل RLS**: إدارة سياق أمان مستوى الصفوف  
✅ **مراقبة الصحة**: نقاط نهاية شاملة للتحقق من الصحة  
✅ **استراتيجية اختبار**: اختبار محلي وتكامل VS Code  

## 🚀 ما التالي

تابع مع **[مختبر 06: تطوير الأدوات](../06-Tools/README.md)** لـ:

- توسيع مجموعة أدوات MCP الخاصة بك  
- تنفيذ أنماط استعلام متقدمة  
- إضافة التحقق من البيانات وتحويلها  
- إنشاء أدوات تحليلية متخصصة  

## 📚 موارد إضافية

### إطار عمل FastMCP
- [وثائق FastMCP](https://github.com/modelcontextprotocol/python-sdk) - دليل FastMCP الرسمي  
- [مواصفات MCP](https://modelcontextprotocol.io/docs/) - مواصفات البروتوكول  
- [دليل تطوير الأدوات](https://modelcontextprotocol.io/docs/tools/) - إنشاء أدوات MCP  

### تكامل قاعدة البيانات
- [وثائق asyncpg](https://magicstack.github.io/asyncpg/current/) - برنامج تشغيل PostgreSQL غير المتزامن  
- [أفضل ممارسات إدارة تجمع الاتصالات](https://www.postgresql.org/docs/current/runtime-config-connection.html) - تحسين PostgreSQL  
- [دليل أمان مستوى الصفوف](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - تنفيذ RLS  

### أنماط FastAPI
- [وثائق FastAPI](https://fastapi.tiangolo.com/) - مرجع إطار العمل  
- [حقن التبعيات](https://fastapi.tiangolo.com/tutorial/dependencies/) - أنماط FastAPI  
- [مهام الخلفية](https://fastapi.tiangolo.com/tutorial/background-tasks/) - إدارة المهام غير المتزامنة  

---

**التالي**: جاهز لتوسيع أدواتك؟ تابع مع [مختبر 06: تطوير الأدوات](../06-Tools/README.md)
