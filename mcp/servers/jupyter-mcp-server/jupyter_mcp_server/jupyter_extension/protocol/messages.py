...
class NotebookListRequest(BaseModel):
    ...
    path: str | None = Field("", description="Directory path to search")
...
class ReadCellsRequest(BaseModel):
    ...
    path: str | None = Field(None, description="Notebook path (uses current if not specified)")
    start_index: int | None = Field(None, description="Start cell index")
    end_index: int | None = Field(None, description="End cell index")
...
class OverwriteCellRequest(BaseModel):
    ...
    new_source: str | list[str] = Field(..., description="New cell source")
...
class ExecuteCellResponse(BaseModel):
    ...
    outputs: list[str | ImageContent] = Field(..., description="Cell outputs")
    execution_count: int | None = Field(None, description="Execution count")
...
class ConnectNotebookRequest(BaseModel):
    ...
    kernel_id: str | None = Field(None, description="Specific kernel ID")
