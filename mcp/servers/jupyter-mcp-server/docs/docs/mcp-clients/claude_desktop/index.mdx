...
To use Jupyter MCP Server with Claude Desktop, add the [Jupyter MCP Server configuration](/mcp-transports/stdio#2-setup-jupyter-mcp-server) to your `claude_desktop_config.json` file, read more on the [MCP documentation website](https://modelcontextprotocol.io/quickstart/user#2-add-the-filesystem-mcp-server).
