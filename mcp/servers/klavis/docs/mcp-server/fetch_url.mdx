---
title: 'Fetch URL'
description: 'Connect AI agents to webpages for content retrieval and analysis'
---
...
Fetch webpages with URL and dump it in HTML, JSON, Markdown, or plain text formats for AI agents.
...
<Tabs>
  <Tab title="API">
    <Steps>
      ...
      <Step title="Create a Strata MCP Server with Fetch URL">
        <CodeGroup>
          ```python Python
          ...
          # Create a Strata MCP server with Fetch URL
          response = klavis_client.mcp_server.create_strata_server(
              servers=[McpServerName.FETCH_URL],
              user_id="user123"
          )
          ```
          ...
          const response = await klavis.mcpServer.createStrataServer({
              servers: [Klavis.McpServerName.FetchUrl],
              userId: "user123"
          });
          ```
      ...
      <Step title="Ready to use">
        <Check>
          🎉 **Your Fetch URL MCP Server is ready!** You can now use your MCP server URL with any MCP-compatible client.
        </Check>
      </Step>
  ...
  <Tab title="UI">
    <Steps>
      ...
      <Step title="Choose Fetch URL">
        Select Fetch URL from the list of available integrations.
      </Step>
  ...
  <Tab title="Open Source">
    <Steps>
      <Step title="Clone repository (for developers to build from source)">
        ...
        cd klavis/mcp_servers/fetch_url
        ```
      </Step>
