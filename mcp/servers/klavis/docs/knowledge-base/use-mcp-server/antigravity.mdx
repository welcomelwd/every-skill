---
title: "Antigravity"
description: "Connect Strata with Antigravity IDE in minutes and supercharge your AI coding experience"
---
<img
  className="block dark:hidden"
  src="/images/knowledge-base/antigravity/antigravity.png"
  alt="Antigravity IDE"
/>
<img
  className="hidden dark:block"
  src="/images/knowledge-base/antigravity/antigravity.png"
  alt="Antigravity IDE"
/>

## Quick Setup Guide

<Steps>
  <Step title="Authorize MCP Servers to Use with Strata">
    Navigate to the [Klavis home page](https://www.klavis.ai/home) and click **"MCP Server"** in the left sidebar. You will see a list of MCP servers available in Klavis.

    <img
      src="/images/knowledge-base/antigravity/step1.png"
      alt="Klavis home page"
    />    
    Click the **"Authorize"** button next to your chosen server. Once server is authorized, you will see **Green Checkmark** status.

    <img
      src="/images/knowledge-base/antigravity/step2.png"
      alt="Authorize MCP Servers"
    />

    <Tip>
      You can authorize one or more servers to use with Strata
    </Tip>

  </Step>

  <Step title="Get Strata Server URL">

    From Klavis Dashboard, click **"Add to Other Clients"** button to get your Strata Server URL.
  
    <img
      src="/images/knowledge-base/antigravity/step3.png"
      alt="Klavis Dashboard"
    />

    **Copy** the strata URL to clipboard - you'll need this in the next step.

    <img
      src="/images/knowledge-base/antigravity/step4.png"
      alt="Copy Strata Server URL"
    />


    <Accordion title="🔐 OAuth Services (Most Common)">
      - Automatically redirected to OAuth authorization
      - Sign in to your account (GitHub, Google, Slack, etc.)
      - Grant necessary permissions
      - Redirected back to Klavis automatically
    </Accordion>
    
    <Accordion title="🔑 API Key Services">
      - Prompted to enter an API key
      - Follow service-specific instructions to generate key
      - Paste key in the provided field
      - Click **"Save"** to continue
    </Accordion>
  </Step>

  <Step title="Configure Antigravity">
    1. Open the MCP store via the "**...**" dropdown at the top of the editor's agent panel.

    <img
      src="/images/knowledge-base/antigravity/step5.png"
      alt="Antigravity IDE Settings"
    />

    2. Click on "**Manage MCP Servers**"
    3. Click on "**View raw config**"

    <img
      src="/images/knowledge-base/antigravity/step6.png"
      alt="Raw Config File in Antigravity IDE Settings"
    />

    4. Modify the **mcp_config.json** with your custom MCP server configuration like the screenshot below.

    <img
      src="/images/knowledge-base/antigravity/step7.png"
      alt="Paste strata URL in mcp_config.json file of Antigravity IDE"
    />

    <Tip>
      You can add **Single** MCP Server URL directly here as well:

      1. Navigate to **Target Server** and click **Three Dots** next to your server
      2. Tap **Show Individual Server URL** and copy the URL
      3. Paste this URL in Antigravity settings as described above

    </Tip>

    
  </Step>

  <Step title="Verify Tools Loading">
     
    <img
      src="/images/knowledge-base/antigravity/step8.png"
      alt="Strata Configuration in Antigravity"
    />
    
    <Note>
      📋 **Reopen Antigravity Settings** to apply the new configuration
    </Note>
    
  </Step>

  <Step title="Start Using in Antigravity Chat">
    <img
      src="/images/knowledge-base/antigravity/step9.jpg"
      alt="Using Strata in Antigravity Chat"
    />
    
    Open Antigravity Chat (`Cmd/Ctrl + Shift + L`) and start using natural language:
    
    <CodeGroup>
    ```text GitHub
    "Create a new issue titled 'Add dark mode' with priority label"
    ```
    
    ```text Slack
    "Send a message to #general: 'Standup meeting in 5 minutes!'"
    ```
    
    ```text Gmail
    "Send email to john@company.com about project update"
    ```
    
    ```text Notion
    "Create a new page called 'Meeting Notes' with today's date"
    ```
    </CodeGroup>
    
    <Tip>
      🎯 Antigravity Chat automatically detects when to use **Strata** based on context - no need to specify explicitly!
    </Tip>
  </Step>

  <Step title="Happy Building! 🎉">
    You're all set! Your **Strata** MCP server is now integrated with Antigravity IDE.
  </Step>

</Steps>

## Troubleshooting

<AccordionGroup>
  <Accordion title="🔧 Tools Not Appearing">
    - Double-check your Server URL for typos
    - Ensure stable internet connection
    - Verify authentication in Klavis dashboard
    - Check Antigravity IDE logs for error messages
    - Try completely restarting Antigravity IDE
  </Accordion>
  
  <Accordion title="🔐 Authentication Issues">
    - Re-authenticate in the Klavis dashboard
    - Check if your OAuth tokens have expired
    - Verify API key permissions (for API key services)
    - Ensure you've granted all necessary permissions
  </Accordion>
  
  <Accordion title="⚡ Performance Issues">
    - Limit the number of active MCP servers
    - Check your internet connection speed
    - Restart Antigravity IDE periodically
    - Contact support if issues persist
  </Accordion>
</AccordionGroup>

## Need Help?

<CardGroup cols={2}>
  <Card title="Community" icon="discord">
    Join our Discord for community support and discussions
  </Card>
  <Card title="Support" icon="headphones">
    Contact our technical support team for assistance
  </Card>
</CardGroup>

---