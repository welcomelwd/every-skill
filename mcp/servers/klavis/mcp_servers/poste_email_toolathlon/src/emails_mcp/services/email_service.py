...
class EmailService:
    ...
    def read_email(self, email_id: str, folder: str = "INBOX") -> EmailMessage:
        """Read specific email by ID"""
        try:
            email_obj = self.imap_backend.fetch_email(email_id, folder=folder)
            
            # Mark as read
            if self.imap_backend.mark_as_read(email_id, folder=folder):
    ...
    def reply_email(self, email_id: str, body: str,
        ...
        try:
            # Get original email
            original_email = self.imap_backend.fetch_email(email_id, folder=folder)
    ...
    def delete_email(self, email_id: str, folder: str = "INBOX") -> bool:
        """Delete email"""
        try:
            self.imap_backend.delete_email(email_id, folder=folder)
            return True
        except Exception as e:
            raise EmailMCPError(f"Failed to delete email: {str(e)}")
    
    def move_email(self, email_id: str, target_folder: str, source_folder: str = "INBOX") -> bool:
        """Move email to another folder"""
        try:
            self.imap_backend.move_email(email_id, target_folder, source_folder=source_folder)
            return True
        except Exception as e:
            raise EmailMCPError(f"Failed to move email: {str(e)}")
    
    def mark_emails(self, email_ids: List[str], status: str, folder: str = "INBOX") -> int:
        ...
        for email_id in email_ids:
            try:
                success = False
                if status == "read":
                    success = self.imap_backend.mark_as_read(email_id, folder=folder)
                elif status == "unread":
                    success = self.imap_backend.mark_as_unread(email_id, folder=folder)
                elif status == "important":
                    success = self.imap_backend.mark_as_important(email_id, folder=folder)
                elif status == "not_important":
                    success = self.imap_backend.mark_as_not_important(email_id, folder=folder)
