...
def register_folder_tools(mcp: FastMCP):
    ...
    def _get_folder_service():
        """Create folder service from per-request config."""
        _, folder_service, _, _ = create_services(get_email_config())
    ...
    async def get_folders() -> str:
        ...
        try:
            folders = _get_folder_service().get_folders()
    ...
    async def create_folder(folder_name: str) -> str:
        ...
        """
        try:
            success = _get_folder_service().create_folder(folder_name)
    ...
    async def delete_folder(folder_name: str) -> str:
        ...
        """
        try:
            success = _get_folder_service().delete_folder(folder_name)
    ...
    async def get_mailbox_stats(folder_name: str = None) -> str:
        ...
        """
        try:
            folder_service = _get_folder_service()
            if folder_name:
    ...
    async def get_unread_count(folder_name: str = None) -> str:
        ...
        """
        try:
            unread_count = _get_folder_service().get_unread_count(folder_name)
