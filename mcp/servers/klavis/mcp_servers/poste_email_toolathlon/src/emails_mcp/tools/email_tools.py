...
from ..server_context import get_email_config, create_services
...
def register_email_tools(mcp: FastMCP):
    ...
    def _get_email_service():
        """Create email service from per-request config."""
        email_service, _, _, _ = create_services(get_email_config())
    ...
    async def get_emails(folder: str = "INBOX", page: int = 1, page_size: int = 20) -> str:
        ...
        """
        try:
            result = _get_email_service().get_emails(folder, page, page_size)
    ...
    async def read_email(email_id: str, folder: str = "INBOX") -> str:
        ...
        Args:
            ...
            folder: Email folder the email is in (default: INBOX)
        """
        try:
            email = _get_email_service().read_email(email_id, folder=folder)
    ...
    async def search_emails(query: str, folder: str = "INBOX", page: int = 1, page_size: int = 20) -> str:
        ...
        """
        try:
            result = _get_email_service().search_emails(query, folder, page, page_size)
    ...
    async def send_email(to: str, subject: str, body: str, html_body: str = None,
        ...
        """
        try:
            success = _get_email_service().send_email(
                to=to,
                subject=subject,
                body=body,
                html_body=html_body,
    ...
    async def reply_email(email_id: str, body: str, html_body: str = None,
                         cc: str = None, bcc: str = None, reply_all: bool = False,
        ...
        """
        try:
            success = _get_email_service().reply_email(
                email_id=email_id,
                body=body,
                html_body=html_body,
                cc=cc,
    ...
    async def forward_email(email_id: str, to: str, body: str = None, html_body: str = None,
                           cc: str = None, bcc: str = None, folder: str = "INBOX") -> str:
        ...
        """
        try:
            success = _get_email_service().forward_email(
                email_id=email_id,
                to=to,
                body=body,
                html_body=html_body,
    ...
    async def delete_email(email_id: str, folder: str = "INBOX") -> str:
        ...
        """
        try:
            success = _get_email_service().delete_email(email_id, folder=folder)
    ...
    @mcp.tool()
    async def move_email(email_id: str, target_folder: str, source_folder: str = "INBOX") -> str:
        """Move email to another folder
        
        Args:
            email_id: Email ID to move
            target_folder: Target folder name
            source_folder: Source folder the email is in (default: INBOX)
        """
        try:
            success = _get_email_service().move_email(email_id, target_folder, source_folder=source_folder)
    ...
    @mcp.tool()
    async def mark_emails(email_ids: List[str], status: str, folder: str = "INBOX") -> str:
        ...
        Args:
            ...
            folder: Email folder the emails are in (default: INBOX)
        """
        try:
            if status not in ['read', 'unread', 'important', 'not_important']:
                return "Error: Status must be 'read', 'unread', 'important', or 'not_important'"
            
            success_count = _get_email_service().mark_emails(email_ids, status, folder=folder)
    ...
    @mcp.tool()
    async def move_emails(email_ids: List[str], target_folder: str, source_folder: str = "INBOX") -> str:
        """Move multiple emails to another folder with improved ID synchronization
        
        Args:
            email_ids: List of email IDs to move
            target_folder: Target folder name
            source_folder: Source folder the emails are in (default: INBOX)
        """
        try:
            success_count = 0
            failed_count = 0
            ...
            for email_id in sorted_ids:
                try:
                    ...
                    success = email_service.move_email(email_id, target_folder, source_folder=source_folder)
                    if success:
                        success_count += 1
    ...
    @mcp.tool()
    async def delete_emails(email_ids: List[str], folder: str = "INBOX") -> str:
        ...
        try:
            ...
            for email_id in sorted_ids:
                try:
                    ...
                    success = email_service.delete_email(email_id, folder=folder)
                    if success:
                        success_count += 1
