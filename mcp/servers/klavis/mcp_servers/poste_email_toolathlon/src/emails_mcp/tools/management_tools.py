...
def register_management_tools(mcp: FastMCP):
    ...
    def _get_services():
        """Create all services from per-request config."""
        return create_services(get_email_config())

    def _get_email_service():
        email_service, _, _, _ = _get_services()
        return email_service

    def _get_draft_service():
        _, _, _, draft_service = _get_services()
    ...
    async def check_connection() -> str:
        ...
        try:
            imap_ok, smtp_ok = _get_email_service().check_connection()
    ...
    @mcp.tool()
    async def get_email_headers(email_id: str, folder: str = "INBOX") -> str:
        ...
        """
        try:
            email = _get_email_service().imap_backend.fetch_email(email_id, folder=folder)
    ...
    async def save_draft(subject: str, body: str, html_body: str = None,
        ...
        """
        try:
            draft_id = _get_draft_service().save_draft(
                subject=subject,
                body=body,
                html_body=html_body,
                to=to,
    ...
    async def get_drafts(page: int = 1, page_size: int = 20) -> str:
        ...
        """
        try:
            result = _get_draft_service().get_drafts(page, page_size)
    ...
    async def update_draft(draft_id: str, subject: str = None, body: str = None,
        ...
        """
        try:
            success = _get_draft_service().update_draft(
                draft_id=draft_id,
                subject=subject,
                body=body,
                html_body=html_body,
    ...
    async def delete_draft(draft_id: str) -> str:
        ...
        """
        try:
            success = _get_draft_service().delete_draft(draft_id)
    ...
    async def download_attachment(email_id: str, attachment_filename: str) -> str:
        ...
        """
        try:
            email = _get_email_service().imap_backend.fetch_email(email_id)
