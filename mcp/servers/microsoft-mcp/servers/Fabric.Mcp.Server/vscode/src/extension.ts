...
function showMcpManualStartNotification() {
    const message = 'Fabric MCP Server settings changed. Please restart the server manually: Command Palette → MCP: List Servers → Fabric MCP → Start';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.lm.registerMcpServerDefinitionProvider('fabricMcpProvider', {
            onDidChangeMcpServerDefinitions: didChangeEmitter.event,
            provideMcpServerDefinitions: async () => {
                // Read enabled MCP services from user/workspace settings
                const config = vscode.workspace.getConfiguration('fabricMcp');
                // Example: ["storage", "keyvault", ...]
                const enabledServices: string[] | undefined = config.get('enabledServices');
                const args = ['server', 'start'];

                // Server Mode (single | namespace | all). Default 'all'.
                const mode = config.get<string>('serverMode') || 'all';
                if (mode) {
                    args.push('--mode', mode);
                }
                ...
                // Only set FABRIC_MCP_COLLECT_TELEMETRY if telemetry is disabled
                const env: Record<string, string | number | null> = {};
                if (!vscode.env.isTelemetryEnabled) {
                    env.FABRIC_MCP_COLLECT_TELEMETRY = 'false';
                }
    ...
    // Listen for changes to fabricMcp.enabledServices and re-register MCP server
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration((event) => {
            if (
                event.affectsConfiguration('fabricMcp.enabledServices') ||
                event.affectsConfiguration('fabricMcp.serverMode') ||
                event.affectsConfiguration('fabricMcp.readOnly')
            ) {
