...
suite('Fabric MCP Extension - activate', () => {
    ...
    test('constructs correct server path for win32', async () => {
        ...
        assert.strictEqual(defs[0].executablePath, path.join('/ext/path', 'server', 'win-x64', 'fabmcp.exe'));
    });
    ...
    test('constructs correct server path for darwin', async () => {
        ...
        const provider = call.args[1];
        const defs = await provider.provideMcpServerDefinitions();
        assert.strictEqual(defs[0].executablePath, path.join('/ext/path', 'server', 'osx-x64', 'fabmcp'));
    });
    ...
    test('constructs correct server path for linux', async () => {
        ...
        const provider = call.args[1];
        const defs = await provider.provideMcpServerDefinitions();
        assert.strictEqual(defs[0].executablePath, path.join('/ext/path', 'server', 'linux-x64', 'fabmcp'));
    });
});
