...
/// <summary>
/// Generator for Node.js PostgreSQL greenfield projects (no existing telemetry)
/// </summary>
public class PostgresNodeJsGreenfieldGenerator : IGenerator
{
    public bool CanHandle(Analysis analysis)
    {
        var postgresProjects = analysis.Projects
            .Count(p => p.AppType == AppType.PostgresNodeJs);
    ...
    {
        var project = analysis.Projects.First(p => p.AppType == AppType.PostgresNodeJs);
        ...
        var builder = new OnboardingSpecBuilder(analysis)
            ...
            .WithDecision(
                ...
                "Node.js application with PostgreSQL. Azure Monitor OpenTelemetry provides automatic instrumentation for HTTP requests, PostgreSQL queries, and custom telemetry.")
            .AddReviewEducationAction(
                ...
                [
                    ...
                    LearningResources.ExamplePostgresSetup
                ])
            .AddPackageAction(
                "add-monitor-package",
...
// PostgreSQL queries will be automatically instrumented
useAzureMonitor({
