...
/// </summary>
public class WinstonNodeJsGreenfieldGenerator : IGenerator
{
    ...
    {
        var project = analysis.Projects.First(p => p.AppType == AppType.WinstonNodeJs);
        ...
        var builder = new OnboardingSpecBuilder(analysis)
            ...
            .WithDecision(
                ...
                "Node.js application with Winston logging. Azure Monitor OpenTelemetry provides automatic instrumentation for HTTP requests and can be configured to collect Winston logs.")
            .AddReviewEducationAction(
                ...
                [
                    ...
                    LearningResources.ExampleWinstonSetup
                ])
            .AddPackageAction(
                "add-monitor-package",
...
// Enable Azure Monitor integration with Winston log collection
useAzureMonitor({
