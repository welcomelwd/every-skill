...
/// <summary>
/// Generator for Node.js MySQL greenfield projects (no existing telemetry)
/// </summary>
public class MySQLNodeJsGreenfieldGenerator : IGenerator
{
    public bool CanHandle(Analysis analysis)
    {
        var mysqlProjects = analysis.Projects
            .Count(p => p.AppType == AppType.MySQLNodeJs);
    ...
    {
        var project = analysis.Projects.First(p => p.AppType == AppType.MySQLNodeJs);
        ...
        var builder = new OnboardingSpecBuilder(analysis)
            ...
            .WithDecision(
                ...
                "Node.js application with MySQL. Azure Monitor OpenTelemetry provides automatic instrumentation for HTTP requests, MySQL queries, and custom telemetry.")
            .AddReviewEducationAction(
                ...
                [
                    ...
                    LearningResources.ExampleMySQLSetup
                ])
            .AddPackageAction(
                "add-monitor-package",
...
// MySQL queries will be automatically instrumented
useAzureMonitor({
