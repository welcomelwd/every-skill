...
public class MemoriesReindexCommandTests : SubscriptionCommandUnitTestsBase<MemoriesReindexCommand, ISreAgentService>
{
    [Fact]
    public void Constructor_InitializesCommandCorrectly()
    {
        var command = Command.GetCommand();
        Assert.Equal("memories_reindex", command.Name);
    ...
    [Fact]
    public async Task ExecuteAsync_TriggersReindex()
    {
        Service.GetAgentAsync(
            Arg.Any<string>(),
            Arg.Any<string?>(),
        ...
        Assert.Contains("reindex", result.Message, StringComparison.OrdinalIgnoreCase);
        await Service.Received(1).ReindexMemoriesAsync(
