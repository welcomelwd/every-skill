...
public class MemoriesAddCommandTests : SubscriptionCommandUnitTestsBase<MemoriesAddCommand, ISreAgentService>
{
    ...
    [Theory]
    [InlineData("--subscription sub --agent myagent --name memory.md --content \"Test content\"", true)]
    ...
    public async Task ExecuteAsync_HandlesServiceErrors()
    {
        ...
        var response = await ExecuteCommandAsync("--subscription", "sub", "--agent", "myagent", "--name", "memory.md", "--content", "Test content");
    ...
    public async Task ExecuteAsync_UploadsMemoryWithContent()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).UploadMemoryAsync(
