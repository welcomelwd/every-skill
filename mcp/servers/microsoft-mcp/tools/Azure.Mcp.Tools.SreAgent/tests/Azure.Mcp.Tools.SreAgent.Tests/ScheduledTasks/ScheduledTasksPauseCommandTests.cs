...
public class ScheduledTasksPauseCommandTests : SubscriptionCommandUnitTestsBase<ScheduledTasksPauseCommand, ISreAgentService>
{
    [Fact]
    ...
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        if (shouldSucceed)
        {
            ...
            Service.PauseScheduledTaskAsync(
                Arg.Any<string>(),
                Arg.Any<string>(),
                Arg.Any<string?>(),
                Arg.Any<CancellationToken>())
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).PauseScheduledTaskAsync(Arg.Any<string>(), "task1", "tenant1", Arg.Any<CancellationToken>());
    }
}
