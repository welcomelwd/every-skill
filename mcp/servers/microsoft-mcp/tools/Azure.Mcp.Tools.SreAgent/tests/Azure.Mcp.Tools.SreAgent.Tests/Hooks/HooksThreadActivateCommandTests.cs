...
public class HooksThreadActivateCommandTests : SubscriptionCommandUnitTestsBase<HooksThreadActivateCommand, ISreAgentService>
{
    ...
    public void Constructor_InitializesCommandCorrectly()
    {
        ...
        Assert.Equal("activate", command.Name);
    ...
    public async Task ExecuteAsync_DeserializationValidation()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        var result = ValidateAndDeserializeResponse(response, SreAgentJsonContext.Default.HooksThreadActivateCommandResult);
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).ActivateThreadHookAsync("https://agent1.azuresre.ai", "thread1", "hook1", null, Arg.Any<CancellationToken>());
    }
}
