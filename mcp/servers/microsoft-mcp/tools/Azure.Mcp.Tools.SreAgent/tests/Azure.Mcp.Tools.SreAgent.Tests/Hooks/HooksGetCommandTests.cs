...
public class HooksGetCommandTests : SubscriptionCommandUnitTestsBase<HooksGetCommand, ISreAgentService>
{
    [Fact]
    ...
    [InlineData("--subscription sub --agent agent1 --name hook1 --tenant tenant1", true)]
    ...
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        if (shouldSucceed)
        {
            ...
            Service.GetHookAsync(
                ...
                Arg.Any<string?>(),
                Arg.Any<CancellationToken>())
                .Returns(new HookEnvelope { Name = "hook1" });
        }
    ...
    public async Task ExecuteAsync_DeserializationValidation()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        var result = ValidateAndDeserializeResponse(response, SreAgentJsonContext.Default.HooksGetCommandResult);
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).GetHookAsync("https://agent1.azuresre.ai", "hook1", null, Arg.Any<CancellationToken>());
    }
}
