...
public class HooksThreadListCommandTests : SubscriptionCommandUnitTestsBase<HooksThreadListCommand, ISreAgentService>
{
    [Fact]
    ...
    [Theory]
    [InlineData("--subscription sub --agent agent1 --thread-id thread1", true)]
    [InlineData("--subscription sub --agent agent1 --thread-id thread1 --tenant tenant1", true)]
    ...
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        if (shouldSucceed)
        {
            ...
            Service.ListThreadHooksAsync(
                ...
                Arg.Any<string?>(),
                Arg.Any<CancellationToken>())
                .Returns(new ThreadHooksResponse());
        }
    ...
    public async Task ExecuteAsync_DeserializationValidation()
    {
        ...
        var response = await ExecuteCommandAsync("--subscription", "sub", "--agent", "agent1", "--thread-id", "thread1");

        Assert.Equal(HttpStatusCode.OK, response.Status);
        var result = ValidateAndDeserializeResponse(response, SreAgentJsonContext.Default.HooksThreadListCommandResult);
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).ListThreadHooksAsync("https://agent1.azuresre.ai", "thread1", null, Arg.Any<CancellationToken>());
    }
}
