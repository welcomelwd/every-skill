...
public class ConnectorsCreateKustoCommandTests : SubscriptionCommandUnitTestsBase<ConnectorsCreateKustoCommand, ISreAgentService>
{
    [Fact]
    public void Constructor_InitializesCommandCorrectly()
    {
        var command = Command.GetCommand();
        Assert.Equal("create_kusto", command.Name);
    ...
    public void RegisterOptions_AddsExpectedOptions()
    {
        ...
        Assert.Contains("--cluster-url", optionNames);
        Assert.Contains("--database", optionNames);
    }

    [Theory]
    [InlineData("--subscription sub --agent agent1 --name connector1 --cluster-url https://cluster.kusto.windows.net", true)]
    [InlineData("--subscription sub --agent agent1 --name connector1 --cluster-url https://cluster.kusto.windows.net --database db1", true)]
    ...
    public async Task ExecuteAsync_DeserializationValidation()
    {
        ...
        var response = await ExecuteCommandAsync("--subscription", "sub", "--agent", "agent1", "--name", "connector1", "--cluster-url", "https://cluster.kusto.windows.net");

        Assert.Equal(HttpStatusCode.OK, response.Status);
        var result = ValidateAndDeserializeResponse(response, SreAgentJsonContext.Default.ConnectorsCreateKustoCommandResult);
