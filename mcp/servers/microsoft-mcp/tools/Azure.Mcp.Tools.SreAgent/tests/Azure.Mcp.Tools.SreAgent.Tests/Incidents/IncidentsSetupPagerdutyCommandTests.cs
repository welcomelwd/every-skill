...
public class IncidentsSetupPagerdutyCommandTests : SubscriptionCommandUnitTestsBase<IncidentsSetupPagerdutyCommand, ISreAgentService>
{
    [Fact]
    ...
    public void RegisterOptions_AddsExpectedOptions()
    {
        ...
        Assert.Contains(command.Options, o => o.Name == $"--{SreAgentOptionDefinitions.ApiKeyEnvName}");
        Assert.Contains(command.Options, o => o.Name == $"--{SreAgentOptionDefinitions.SubdomainName}");
    }

    [Theory]
    [InlineData("--subscription sub --agent agent1 --name connector1 --api-key-env KEY_VAR", true)]
    ...
    public async Task ExecuteAsync_HandlesServiceErrors()
    {
        ...
        var response = await ExecuteCommandAsync("--subscription", "sub", "--agent", "agent1", "--name", "connector1", "--api-key-env", "KEY_VAR");
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        var response = await ExecuteCommandAsync("--subscription", "sub", "--agent", "agent1", "--name", "connector1", "--api-key-env", "KEY_VAR", "--tenant", "tenant1");
