...
public class IncidentsActiveListCommandTests : SubscriptionCommandUnitTestsBase<IncidentsActiveListCommand, ISreAgentService>
{
    [Fact]
    ...
    public async Task ExecuteAsync_ValidatesInputCorrectly(string args, bool shouldSucceed)
    {
        if (shouldSucceed)
        {
            ...
            Service.ListIncidentThreadsAsync(
                Arg.Any<string>(),
                Arg.Any<string?>(),
                Arg.Any<CancellationToken>())
                .Returns(new List<ThreadListItem>());
        }
    ...
    public async Task BindOptions_BindsOptionsCorrectly()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.Status);
        await Service.Received(1).ListIncidentThreadsAsync(Arg.Any<string>(), "tenant1", Arg.Any<CancellationToken>());
    }
}
