...
resource aiDeveloperRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(azureAiDeveloperRoleId, testApplicationOid, aiServicesAccount.id)
  scope: aiServicesAccount
  properties: {
...
resource aiUserRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(azureAiUserRoleId, testApplicationOid, aiServicesAccount.id)
  scope: aiServicesAccount
  properties: {
...
// Dynamic resource outputs for test configuration
output openAIAccount string = aiServicesAccount.name
output openAIDeploymentName string = gpt4oMiniDeployment.name
output openAIAccountResourceGroup string = resourceGroup().name
