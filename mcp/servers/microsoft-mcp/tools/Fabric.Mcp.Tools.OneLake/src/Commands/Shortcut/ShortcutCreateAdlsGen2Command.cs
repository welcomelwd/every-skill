    ...
    : AuthenticatedCommand<ShortcutCreateAdlsGen2Options, OneLakeShortcut>()
{
    ...
    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, ShortcutCreateAdlsGen2Options options, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Error creating ADLS Gen2 shortcut. Workspace: {Workspace}, Item: {Item}.", options.WorkspaceId, options.ItemId);
