...
| `AWS_API_MCP_SUPPRESS_DEPRECATION_WARNING`                        | ❌ No                       | `"false"`                                                | When set to "true", suppresses the deprecation notice that is otherwise shown at server startup, in the server instructions, and in tool descriptions. The notice recommends migrating to the AWS MCP Server; see the [migration guide](https://github.com/awslabs/mcp/blob/main/src/aws-api-mcp-server/MIGRATION.md).                                                                                                                                                                                                                                                                                                            |
...
- **Direct Arguments Only**: The `AWS_API_MCP_ALLOW_UNRESTRICTED_LOCAL_FILE_ACCESS` flag enforces access control only on file paths passed **directly** as arguments to the AWS CLI command. It does **not** extend to files that are referenced **indirectly** from within those arguments. When a file passed as a direct argument itself references other local files, those nested paths are resolved by the AWS CLI and are **not** subject to the flag's access control.
