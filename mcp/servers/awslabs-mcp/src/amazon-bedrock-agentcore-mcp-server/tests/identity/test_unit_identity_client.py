...
"""Unit tests for Identity client wrapper."""

from awslabs.amazon_bedrock_agentcore_mcp_server.tools.identity.identity_client import (
    MCP_USER_AGENT,
    _control_clients,
...
PATCH_SESSION = (
    'awslabs.amazon_bedrock_agentcore_mcp_server.tools.identity.identity_client.boto3.Session'
)
...
class TestGetControlPlaneClient:
    ...
    def test_creates_client_with_defaults(self, mock_session_cls):
        """Creates client with default region and correct service name."""
    ...
    def test_caches_client(self, mock_session_cls):
        """Returns cached client on subsequent calls with the same region."""
        mock_session = MagicMock()
        mock_session.client.return_value = MagicMock()
        mock_session_cls.return_value = mock_session
    ...
    def test_user_agent_tracking(self, mock_session_cls):
        """Control plane client has the identity user-agent."""
        mock_session = MagicMock()
        mock_session.client.return_value = MagicMock()
        mock_session_cls.return_value = mock_session
        ...
        assert 'identity' in config.user_agent_extra
