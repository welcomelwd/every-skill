...
"""Unit tests for Gateway client wrapper."""

from awslabs.amazon_bedrock_agentcore_mcp_server.tools.gateway.gateway_client import (
    MCP_USER_AGENT,
    _control_clients,
...
PATCH_SESSION = (
    'awslabs.amazon_bedrock_agentcore_mcp_server.tools.gateway.gateway_client.boto3.Session'
)
...
class TestGetControlPlaneClient:
    ...
    def test_user_agent_tracking(self, mock_session_cls):
        """Client is configured with the gateway user-agent."""
        mock_session = MagicMock()
        mock_session.client.return_value = MagicMock()
        mock_session_cls.return_value = mock_session
        ...
        assert 'gateway' in config.user_agent_extra
