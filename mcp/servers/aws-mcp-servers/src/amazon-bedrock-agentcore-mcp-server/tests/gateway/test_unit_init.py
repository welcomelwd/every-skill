...
"""Unit tests for Gateway tools registration."""
...
class TestRegisterGatewayTools:
    """Tests for register_gateway_tools."""

    def test_registers_all_groups(self):
        """Registers all 15 gateway tools without error."""
        ...
        register_gateway_tools(mock_mcp)

        # Verify tool() was called for each of the 15 tools:
        # 5 gateways + 6 targets + 3 resource policy + 1 guide
    ...
    def test_raises_runtime_error_on_failure(self):
        ...
        with pytest.raises(RuntimeError, match='Failed to register gateway'):
            register_gateway_tools(mock_mcp)
