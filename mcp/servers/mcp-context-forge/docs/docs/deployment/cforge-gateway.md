...
The `cforge` CLI is installed with ContextForge package:
...
- **gateway**: Defines ContextForge configuration - can use a pre-built image or build from a Git repository
...
PostgreSQL and Redis are **automatically deployed** with ContextForge stack using hardcoded defaults:
...
### Understanding mTLS in ContextForge
