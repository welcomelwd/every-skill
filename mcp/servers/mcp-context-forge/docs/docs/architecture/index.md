...
!!! warning "Deprecated Rust runtime sidecars"
    The Rust MCP and Rust A2A runtime sidecars are deprecated as of
    2026-06-11 and sunset on 2026-07-07. New deployments should use the
    default Python runtime paths. See [Deprecations](../deprecations.md).
...
- Supports **PostgreSQL** (via psycopg3) and **SQLite** (default, file-based)
...
!!! warning "Deprecated optional Rust MCP runtime"
    The optional Rust MCP runtime sidecar mentioned below is deprecated. Prefer
    the Python MCP transport unless maintaining an existing deployment.
...
- Optional **MCP Apps support** for `ui://` resources, UI metadata, and
  short-lived AppBridge tool calls. See [MCP Apps](mcp-apps.md).
